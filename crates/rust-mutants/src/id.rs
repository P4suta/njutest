...
//!
//! An identity is deliberately over-specified. Path, rule, and span alone
//! would collide across edits of the same file, so three digests pin the
//! mutant to exact bytes: the source digest changes when anything in the
//! file changes (a CRLF-to-LF conversion included), the original digest pins
//! what is being replaced, and the replacement digest pins what replaces it.
//! A cached outcome can therefore never be reused for a mutant that is not
//! literally the same edit to the same bytes.

use sha2::{Digest as _, Sha256};

use crate::span::{Span, SpanError};

/// The domain separator hashed first for every mutant ID. It carries the
/// recipe version: a future recipe becomes `rust-mutants-id-v2` so that v1
/// identities can never be mistaken for v2 identities.
...
/// The length of the short ID shown in the console and accepted by
/// `--mutant`. The catalog builder proves the prefix is unique within a run.
pub const DISPLAY_ID_LENGTH: usize = 20;

/// The shortest `--mutant` prefix the catalog resolves. Anything shorter is
/// rejected as a typo rather than silently matching half the run.
...
pub struct Identity {
    /// The workspace-relative source path with forward slashes, as produced
    /// by [`normalize_path`].
    pub path: String,
    /// The operator rule name, for example `eq-to-neq`.
    pub rule_name: String,
    /// The rule's version. Bumping it re-mints every mutant the rule
    /// produces, which is how a behaviour change invalidates cached outcomes.
    ...
    /// The SHA-256 of the replacement bytes. For a deletion, the digest of
    /// the empty string.
...
/// Converts a source path to the canonical workspace-relative form used in
/// identities and reports: forward slashes, cleaned, no leading `./`.
///
/// Backslashes are treated as separators so that a Windows path lands on the
/// same identity as the POSIX spelling of the same file: a mutant must not
/// change its name because the run happened on a different operating system.
///
/// # Errors
///
...
pub fn normalize_path(path: &str) -> Result<String, PathError> {
    ...
    // inputs such as "./C:" cannot turn into a volume name only after the
    // first check has passed.
...
/// A volume name is an ASCII letter followed by a colon. Both halves are
/// checked: a colon after something that is not a letter (`1:`) is a
/// directory whose name contains a colon, which POSIX allows.
fn has_volume_name(path: &str) -> bool {
    let bytes = path.as_bytes();
    matches!((bytes.first(), bytes.get(1)), (Some(letter), Some(b':')) if letter.is_ascii_alphabetic())
}

/// The relative-path half of Go's `path.Clean`: drop empty and `.` elements,
/// resolve `..` against a preceding element, keep a leading `..`, and answer
/// `.` for a path that cleans to nothing.
...
impl Identity {
    /// Whether the identity is complete and canonical. Every field that feeds
    /// the hash is checked, because a malformed field would otherwise produce
    /// a plausible-looking ID for a mutant that cannot be resolved back to a
    /// source location.
    ///
    /// # Errors
    ///
    ...
    ///
    /// ```text
    /// enc("rust-mutants-id-v1")
    /// enc(path)                   workspace-relative, forward slashes
    /// enc(rule_name)
    ...
    /// followed by those bytes. Numbers are hashed as decimal text and digests
    /// as hex text, so the encoding is fully described by the bytes on the
    /// wire and can be reimplemented in any language.
    ///
    /// # Errors
    ///
...
/// Appends `enc(s)` to `hasher`: a 4-byte big-endian byte length followed by
/// the raw bytes. Every hash the engine builds from a list of fields uses
/// this one encoding.
///
/// # Errors
///
...
/// The short form of a full mutant ID. Uniqueness of the short form is a
/// property of a whole catalog, proven by the catalog builder; this only
/// truncates.
///
/// # Errors
///
...
/// Whether every character of `s` is a lowercase hex digit. Uppercase is
/// rejected rather than folded: identities are compared as strings, so
/// exactly one spelling may exist.
#[must_use]
