...
{{ALLOW}}mod {{MODULE}} {
    ...
    const INDICES: &[u32] = &[
{{INDICES}}    ];
    static SEEN: [__rm_std::sync::atomic::AtomicBool; {{SLOTS}}] = [const { __rm_std::sync::atomic::AtomicBool::new(false) }; {{SLOTS}}];
    static HEADER: __rm_std::sync::atomic::AtomicBool = __rm_std::sync::atomic::AtomicBool::new(false);
    ...
    impl<T: Observable + __rm_std::default::Default + __rm_std::cmp::PartialEq> Probe for &T {
        fn probed(&self) -> bool {
            **self == <T as __rm_std::default::Default>::default()
        }
    }
    ...
    pub(crate) fn infect(slot: usize, index: u32) {
        ...
        if SEEN[slot].swap(true, __rm_std::sync::atomic::Ordering::Relaxed) {
            return;
        }
        record(index);
    }
    ...
    #[cold]
    fn record(index: u32) {
        let path = match __rm_std::env::var("{{PROBE_ENV}}") {
            __rm_std::result::Result::Ok(value) if !value.is_empty() => value,
            _ => return,
        };
        let opened = __rm_std::fs::OpenOptions::new().create(true).append(true).open(&path);
        let mut file = match opened {
            __rm_std::result::Result::Ok(file) => file,
            __rm_std::result::Result::Err(_) => __rm_std::process::exit({{EXIT}}),
        };
        if !HEADER.swap(true, __rm_std::sync::atomic::Ordering::Relaxed) {
            let header = __rm_std::format!("{{SCHEMA}} {} {}\n", CATALOG, COUNT);
            if __rm_std::io::Write::write_all(&mut file, header.as_bytes()).is_err() {
                __rm_std::process::exit({{EXIT}});
            }
        }
        let line = __rm_std::format!("{}\n", index);
...
///
/// Every path is absolute (`__rm_std::…`) so that a crate which renamed or
