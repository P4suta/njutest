...
fn report(run: &str, outcomes: &[(&str, &str)]) -> BuildReport {
    ...
    report.mutants = outcomes
        ...
        .map(|((id, outcome), catalog_index)| {
            ...
            MutantRecord {
                catalog_index: njutest_cli::report::CatalogIndex::new(catalog_index),
                id: id.repeat(64),
...
fn completed(run: &str, measured: Vec<(String, BuildSelection, BuildReport)>) -> Report {
    let measurements = njutest_cli::report::across::BuildMeasurements::checked(measured)
        .expect("checked build measurements");
    let latticed = configured(&run_id(run), &measurements).expect("builds of one catalog");
    let njutest_cli::report::LatticedDocument::Complete(whole) = latticed else {
...
fn nothing_to_reconcile_is_refused_rather_than_invented() {
    let refusal = njutest_cli::report::across::BuildMeasurements::checked(Vec::new());
    assert_eq!(
        refusal,
        Err(njutest_cli::report::across::BuildMeasurementsError::Empty),
...
fn one_builds_acceptance_cannot_answer_another_builds_survivor() {
    ...
    assert_eq!(whole.verdict(), njutest_cli::report::Verdict::Insufficient);
    assert_eq!(conclusion.findings.len(), 1);
    assert_eq!(
        conclusion.findings[0].origin,
        njutest_cli::report::FindingOrigin::Source {
            build: name("release"),
            run_id: run_id("r-1"),
