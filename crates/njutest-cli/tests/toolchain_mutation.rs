...
fn every_mutant_is_routed_to_the_tests_that_reach_it_and_no_others() {
    ...
    let events = njutest_cli::trace::read_events(std::io::BufReader::new(
        std::fs::File::open(recording.join(njutest_cli::trace::FILE_NAME)).expect("the stream"),
    ))
    .expect("the events");
...
fn a_test_that_writes_into_the_tree_while_it_is_measured_is_said_to_have_done_so() {
    ...
    let named: Vec<&str> = report["limitations"]
        ...
        .collect();
    assert!(
        named.contains(&njutest_cli::limitation::TREE_WRITTEN_DURING_MEASUREMENT),
    ...
    let detail = report["limitations"]
        ...
        .and_then(|all| {
            all.iter()
                .find(|one| one["name"] == njutest_cli::limitation::TREE_WRITTEN_DURING_MEASUREMENT)
        })
