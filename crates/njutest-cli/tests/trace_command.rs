...
fn recorded(root: &std::path::Path, run: &str, events: &[Event]) {
    ...
    std::fs::write(directory.join(njutest_cli::trace::FILE_NAME), stream).expect("the recording");
}
...
fn whole(run: &str) -> Vec<Event> {
    vec![
        at(
            1,
            Payload::RunStart {
                start: njutest_cli::trace::StartRecord::of(
                    run,
        ...
        at(
            5,
            Payload::RunEnd {
                run: njutest_cli::trace::RunRecord {
...
fn a_recording_that_cannot_be_read_is_a_refusal_and_never_an_empty_run() {
    ...
    std::fs::create_dir_all(&directory).expect("a directory for the recording");
    std::fs::write(
        directory.join(njutest_cli::trace::FILE_NAME),
