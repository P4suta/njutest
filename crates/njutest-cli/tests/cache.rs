...
fn zero_ttl_and_zero_size_bound_both_mean_unbounded() {
    ...
    assert_eq!(collected, njutest_cli::cache::store::Collected::default());
...
fn blank_import_lines_are_skipped_without_hiding_later_answers_or_line_numbers() {
    ...
    let kept = store(dir.path());
    let one =
        njutest_cli::report::json::line(&report("run-1", &"1".repeat(64))).expect("first line");
    let two =
        njutest_cli::report::json::line(&report("run-2", &"2".repeat(64))).expect("second line");
    let stream = format!("\n{one}\n\n{two}\n");
