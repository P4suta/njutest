...
use njutest_cli::assure::identity::{Asked, Machine, inputs, of};
use njutest_cli::config::Config;
use njutest_cli::evidence::digest::Mode;
use njutest_cli::evidence::key::Common;
...
fn a_plan_compiles_what_the_configuration_says_to_compile() {
    ...
    let arguments = njutest_cli::cli::Plan {
        directory: None,
        packages: Vec::new(),
        why: false,
        offline: true,
    ...
    let selection =
        njutest_cli::app::plan::compiled(repo.root(), &arguments).expect("the configuration reads");
