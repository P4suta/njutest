...
fn populated_lattice_from(
    measurements: &njutest_cli::report::across::BuildMeasurements,
) -> Result<LatticedDocument, njutest_cli::report::across::ConfiguredError> {
    njutest_cli::report::across::configured(&run_id("20260905t081500z-abcdef"), measurements)
}
...
fn populated_draft(vary: &dyn Fn(&mut BuildReport)) -> BuildReport {
    ...
    source.provenance = njutest_cli::report::Provenance {
        identity: "f".repeat(64),
        facts: njutest_cli::report::Established::Here,
    };
    source.candidates = vec![njutest_cli::report::CandidateRecord {
        finding: "a".repeat(64),
        mutant: "a".repeat(64),
        kind: "patch".to_owned(),
    ...
    source.seams = vec![njutest_cli::report::SeamRecord {
        id: "c".repeat(64),
    ...
    source.resources = vec![njutest_cli::report::ResourceRecord {
    ...
    source.scope = Scope {
        ...
        configured_builds: vec![njutest_cli::config::DEFAULT_CONFIGURATION.to_owned()],
        shard: None,
    };
...
fn populated_shard_source() -> BuildReport {
    populated_draft(&|source| {
        ...
        source.mutants[0].catalog_index = njutest_cli::report::CatalogIndex::new(1);
    })
}
...
fn mutant_rows_rederive_every_counter_and_refuse_duplicate_identities() {
    ...
    let duplicate = populated_varying(&|source| {
        ...
        repeated.catalog_index = njutest_cli::report::CatalogIndex::new(1);
...
fn every_survivor_and_affirmative_model_outcome_has_exactly_one_model_record() {
    for outcome in [
        njutest_cli::report::Decided::Survived,
        njutest_cli::report::Decided::ModelNoticed,
        njutest_cli::report::Decided::ModelProved,
    ] {
        let refused = populated_varying(&|source| {
            source.contract = njutest_cli::config::Contract::VerifiedV1;
            source.mutants[0].outcome = outcome.clone();
            source.mutants[0].reuse =
                njutest_cli::report::Reuse(njutest_cli::report::Established::Here);
            source.accounting.mutants = match &outcome {
                njutest_cli::report::Decided::Survived => MutantAccounting {
                    cataloged: 1,
                    executed: 1,
                    survived: 1,
                    observers: ObserverAccounting {
                ...
                njutest_cli::report::Decided::ModelNoticed => MutantAccounting {
                    cataloged: 1,
                    executed: 1,
                    model_noticed: 1,
                    observers: ObserverAccounting {
                ...
                njutest_cli::report::Decided::ModelProved => MutantAccounting {
                    cataloged: 1,
                    executed: 1,
                    model_proved: 1,
                    observers: ObserverAccounting {
                ...
                njutest_cli::report::Decided::CompileRejected
                | njutest_cli::report::Decided::Killed { .. }
                | njutest_cli::report::Decided::StepLimitReached { .. }
                | njutest_cli::report::Decided::Waited { .. }
                | njutest_cli::report::Decided::Unreached
                | njutest_cli::report::Decided::Equivalent
                | njutest_cli::report::Decided::Unconfirmed { .. }
                | njutest_cli::report::Decided::Errored { .. } => {
                    panic!("the model phase refuses a {outcome:?} row")
                }
            };
            if outcome == njutest_cli::report::Decided::Survived {
        ...
        assert!(
            match outcome {
                njutest_cli::report::Decided::Survived => matches!(
                    refused,
                    FixtureError::Completion(njutest_cli::report::CompletionError::ModelRequired)
                ),
                _ => matches!(
                    refused,
                    FixtureError::Configured(
                        njutest_cli::report::across::ConfiguredError::ModelPipelineRequired
                    )
                ),
            },
...
fn contracts_without_model_checking_refuse_model_outcomes() {
    for contract in [
        ...
        let refused = populated_varying(&|source| {
            ...
            source.mutants[0].outcome = njutest_cli::report::Decided::ModelNoticed;
        })
