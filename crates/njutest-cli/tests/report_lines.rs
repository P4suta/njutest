...
fn a_run_says_where_it_wrote_so_nobody_reading_it_has_to_know_the_layout() {
    let document = "elsewhere/runs/20260101T000000Z-aaaaaa/njutest-assurance-report-v2.json";
    let text = lines::kept(&report(), document).expect("the checked report keeps its document");
    let said = records(&text, "REPORT");
    assert_eq!(
        said,
        vec![
            "REPORT\telsewhere/runs/20260101T000000Z-aaaaaa/njutest-assurance-report-v2.json"
                .to_owned()
        ],
