...
//! Every enum of the runner that spells its wire name twice, and that the two agree.
...
fn every_wire_name_a_value_carries_is_the_one_serde_writes() {
    agrees!(
        njutest_cli::report::Verdict,
        njutest_cli::report::Blind,
        njutest_cli::report::FindingKind,
        njutest_cli::wire::rule::Rule,
    );
}
