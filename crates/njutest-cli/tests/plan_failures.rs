...
fn a_plan_builds_in_its_dedicated_scratch_target_and_raii_removes_it() {
    ...
    assert!(
        scratch
            ...
            .is_some_and(|name| name.starts_with(njutest_cli::scratch::DIR_PREFIX)),
        "{}",
        scratch.display()
    );
    assert!(
...
fn build_defaults_command_environment_limit_and_trace_are_exact() {
    ...
    assert_eq!(
        ...
        Some(
            scratch
                .join(njutest_cli::build::BUILD_PROFILES)
                .join("%p-%m.profraw")
                .display()
                .to_string()
        )
