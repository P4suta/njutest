...
const READERS: [(&str, Reader); 15] = [
    ...
    ("carried_answers", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
            text.lines()
                ...
                .all(|line| njutest_cli::report::json::parse(line).is_ok())
        })
    }),
    ("config", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
            njutest_cli::config::Config::parse(text, Path::new(".njutest.toml")).is_ok()
        })
    }),
    ("coverage_export", |data| {
        njutest_cli::coverage::parse_export(data).is_ok_and(|files| !files.is_empty())
    }),
    ("depinfo", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
    ...
    ("offered_candidates", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
            njutest_cli::repair::take(text, Path::new("/w"), &njutest_cli::repair::allowed(&[]))
                .is_ok()
        })
    }),
    ("report_document", |data| {
        std::str::from_utf8(data).is_ok_and(|text| njutest_cli::report::json::parse(text).is_ok())
    }),
    ("touch_log", |data| {
        std::str::from_utf8(data)
