...
fn kept(it: &Verified) {
    ...
    for name in [
        ...
        njutest_cli::app::reports::SARIF_NAME,
        njutest_cli::app::reports::JUNIT_NAME,
...
fn an_acceptance_is_written_where_the_next_run_reads_it() {
    ...
    assert!(
        accepted.out.contains("accepted") && accepted.out.contains(njutest_cli::config::FILE_NAME),
        "it says what it did and where, because an acceptance written somewhere a person \
         is not looking is one they record twice: {}",
        accepted.out
    );
    let written = std::fs::read_to_string(it.root.join(njutest_cli::config::FILE_NAME))
    ...
    let config = njutest_cli::config::Config::load(&it.root).expect("a configuration it can read");
    let recorded = config
        .acceptance
        .iter()
        .find(|one| {
    ...
    let after = njutest_cli::config::Config::load(&it.root).expect("a configuration it can read");
    assert_eq!(
...
fn refusals(it: &Verified, survivor: &str) {
    ...
    let configured = it.root.join(njutest_cli::config::FILE_NAME);
    ...
    assert!(
        unparsable.code != 0 && unparsable.err.contains(njutest_cli::config::FILE_NAME),
...
fn accept_propagates_both_a_missing_run_and_an_unreadable_report() {
    ...
    assert!(
        unreadable.err.contains(run)
            && unreadable
                .err
                .contains(njutest_cli::app::reports::DOCUMENT_NAME),
        "{}",
        unreadable.err
    );
}
...
fn bundled(it: &Verified) {
    ...
    let manifest =
        std::fs::read_to_string(directory.join(njutest_cli::app::diagnostics::MANIFEST_NAME))
    ...
    assert!(
        directory.join(njutest_cli::trace::FILE_NAME).exists(),
...
fn stored(it: &Verified) {
    std::fs::write(
        it.root.join(njutest_cli::config::FILE_NAME),
        "version = 1\n\n[cache]\nmax_bytes = 1\n",
    )
...
fn a_configuration_is_written_once_and_never_over_one_somebody_wrote() {
    ...
    assert!(
        written.out.contains(njutest_cli::config::FILE_NAME),
        "it says which file it wrote, because a command that writes in silence is one a \
         person runs again: {}",
        written.out
    );
    let path = root.join(njutest_cli::config::FILE_NAME);
    assert_eq!(
