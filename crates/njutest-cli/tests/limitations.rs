...
use njutest_cli::assure::run::limitation_detail;
...
fn the_limitations_page_names_every_limitation_the_runner_can_state() {
    ...
    let missing: Vec<&str> = njutest_cli::limitation::ALL
        .into_iter()
...
fn the_names_a_run_states_are_the_names_the_register_holds() {
    ...
    let held: std::collections::BTreeSet<&str> = njutest_cli::limitation::ALL.into_iter().collect();
    ...
    assert_eq!(
        held.len(),
        njutest_cli::limitation::ALL.len(),
...
fn every_limitation_either_product_can_state_is_one_a_test_puts_to_something() {
    ...
    let every: Vec<&str> = rust_mutants::limitation::ALL
        .into_iter()
        .chain(njutest_cli::limitation::ALL)
        .collect();
