...
enum ChangeError {
    ...
    /// The configured report directory cannot be represented exactly in the
    /// exclusion protocol shared with git and the evidence walk.
    #[error(transparent)]
    Evidence(#[from] crate::evidence::tree::ScanError),
}

/// Why a requested durable trace could not be established before verification
/// began.
#[derive(Debug, thiserror::Error)]
enum TraceSetupError {
    ...
    /// The common build namespace could not be claimed inside the new outer
    /// recording.
    ...
    /// A requested engine trace could not be made durable at the end of its
    /// configured build.
...
/// Why configured-build evidence could not take its one legal terminal
/// transition into a durable report document.
#[derive(Debug, thiserror::Error)]
...
/// A completion failure, distinguished by whether abandoning its private
/// publication namespace failed too.
#[derive(Debug, thiserror::Error)]
...
/// A terminal report plus exclusive ownership of the directory into which it
/// and any model artifacts will be persisted.
struct Reconciled {
...
///
/// # Errors
/// Returns an output failure when the command's diagnostic or report stream
...
/// Why a completed document could not finish its durable publication and
...
/// Retention is a derived, post-publication maintenance step. Its failure can
/// never turn an already-durable report back into a failed publication.
...
/// The directory under a runner trace that owns all configured-build engine
/// traces.
pub use crate::app::trace::BUILDS_DIRECTORY;

/// A configured build ready to execute after every requested trace namespace
...
/// The only two pre-execution engine trace states. A requested trace cannot
/// degrade into the disabled state.
enum EngineTrace {
    Disabled,
    Requested {
...
/// Captures every configured build and, when requested, exclusively creates
/// every ordinal trace namespace before the first build starts.
