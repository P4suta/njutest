...
///
/// A report store can only be derived from this value, so parsing
/// `.njutest.toml` and later opening the configured store share one directory
/// identity instead of reopening a mutable pathname.
#[derive(Debug)]
...
///
/// The only constructor opens it beneath a held [`StoreRoot`], and every census
/// and run opening goes through the same value, so a run is never validated
/// against one `runs` inode and read from another.
#[cfg(unix)]
#[derive(Debug)]
struct RunsRoot {
    directory: std::fs::File,
...
/// One stored run held open as the exact directory selected by an index or
...
///
/// The private fields make it impossible for a persistence caller to claim
/// that an arbitrary pre-existing directory was reserved for this report.
/// The canonical run path does not exist until this value is consumed by the
/// checked publication transition.
#[derive(Debug)]
...
/// A report tree whose producer-visible pathname has been retired from the
/// write protocol. Only this state can cross the publication transition.
///
/// The retained artifact ledger is re-read through the held directory handle
/// after the no-replace rename, before either parent is synced or ownership is
/// disarmed.
#[derive(Debug)]
...
/// Open directory capabilities retained from exclusive creation through
/// publication. On Unix every model read, report write, tree sync, rename,
/// and parent sync is relative to these descriptors rather than a spelling
/// that an ancestor replacement could redirect.
#[cfg(unix)]
#[derive(Debug)]
...
/// Hosts without the Unix handle-relative backend refuse publication. A path
/// fallback cannot bind check, read, rename, and cleanup to the same object.
#[cfg(not(unix))]
#[derive(Debug)]
struct RunCapability;
...
///
/// Keeping this as a closed state instead of a boolean matters after the
/// no-replace rename: cleanup must remove the canonical directory rather than
/// the now-absent staging spelling until its parent has been synced.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum RunOwnership {
    Staging,
    Published,
    /// The canonical directory is durable and may already be named by a
    /// derived index. It is no longer legal to remove it while unwinding a
    /// later validation or index failure.
    Committed,
    Disarmed,
}
...
impl RunDirectory {
    ...
    ///
    /// # Errors
    /// Returns the cleanup failure rather than pretending a partial staging
    /// tree was removed.
...
impl RunCapability {
    ...
    /// Proves the name `runs` under the held report root still names the
    /// publication parent this capability retains.
    ///
    /// # Errors
...
/// Reads one explicitly named configuration without following its final
/// component and from the same descriptor whose kind and length are checked.
///
/// # Errors
...
impl WorkspaceRoot {
    ...
    ///
    /// # Errors
    /// Refuses non-regular, symlinked, oversized, non-UTF-8, malformed, or
    /// unreadable configuration bytes.
    ...
    /// Derives a report store from the same workspace object that supplied
    /// the configuration.
    ///
    /// # Errors
...
impl StoredRun {
    ...
    ///
    /// # Errors
    /// Refuses symlinks, non-regular entries, oversized bytes, concurrent
    /// changes, non-UTF-8 text, and unsupported hosts.
...
impl Store {
    ...
    ///
    /// # Errors
    /// Refuses an unreadable, malformed, or invalid configuration instead of
    /// silently selecting the default report root.
    ...
    ///
    /// # Errors
    /// Refuses an existing entry, an ASCII-case alias, or any filesystem
    /// failure. A model phase therefore cannot write into a stale or
    /// pre-populated report namespace.
    #[cfg(unix)]
    ...
    /// The canonical run identity one index names, retained only for
    /// authority-internal retention and tests.
    ///
    /// # Errors
    /// Returns the closed index error when the pointer or run namespace is
    /// unreadable, malformed, or unsafe.
    ...
    /// Publishes one complete report through this already-held workspace and
    /// store capability.
    ///
    /// # Errors
...
pub enum StoreError {
    ...
    /// This non-Unix host lacks the handle-relative filesystem backend needed
    /// for authoritative report reads, writes, deletion, and publication.
    #[error(
...
pub enum PublicationFailure {
    /// The host cannot bind authoritative report bytes strongly enough to
    /// publish them.
...
pub struct Kept {
    ...
    /// Whether the derived latest-run indexes were updated after the
    /// canonical directory became durable.
    pub indexes: IndexPublication,
}

/// The non-authoritative index side of a durable report publication.
///
/// The canonical run directory is the authority. Filesystem APIs cannot
/// atomically rename that directory and two independent pointer files as one
/// transaction, so an index failure after the directory rename is a
/// successful publication with a repairable index failure, never
/// `REPORT_NOT_KEPT`.
#[derive(Debug)]
pub enum IndexPublication {
    /// Shards are merge inputs and deliberately update no latest-complete
    /// pointer.
    NotRequired,
    ...
    /// A derived index may already name the durable run, but the held run no
    /// longer matched its sealed ledger after the index update. The canonical
    /// directory is deliberately retained: deleting it would leave a durable
    /// pointer naming a missing run.
    RunChanged {
...
/// written then — and [`StoreError::NotKept`] when the authoritative run
/// directory cannot be published. A derived index failure is returned in
/// [`Kept::indexes`] because it cannot undo an already-durable authority.
#[cfg(feature = "testkit")]
...
///
/// A shard is retained only as canonical merge input: it does not update the
/// latest-complete indexes and does not acquire complete-report projections.
///
/// # Errors
/// [`StoreError::Report`] when the document cannot be serialized or audited,
/// and [`StoreError::NotKept`] when the authoritative run directory cannot be
/// published. A derived index failure is returned in [`Kept::indexes`].
#[cfg(feature = "testkit")]
...
/// Writes a document into the exact directory exclusively claimed before a
/// post-lattice model phase began.
///
/// # Errors
/// Refuses a claim for another root or run, an unsound document, or any I/O
/// failure.
...
///
/// # Errors
/// A run-directory entry could not be read, so no partial view is used to
/// decide what may be removed.
#[cfg(feature = "testkit")]
...
/// Quarantines one retained run under an exclusive token and proves the
/// quarantined directory is the one the census opened.
///
/// # Errors
...
/// Destroys one quarantined run and removes its quarantine name, proving the
/// name still matches the opened directory on both sides of the removal.
///
/// # Errors
...
/// Which index names `candidate` now, read after the candidate is unreachable
/// by name so a pointer written since the protection census is seen.
///
/// # Errors
...
///
/// # Errors
/// Returns the closed index error when the index or named run directory is
/// unreadable, malformed, or unsafe to follow.
#[cfg(feature = "testkit")]
