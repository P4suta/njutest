...
/// One scoped worker whose normal return is joined explicitly. The lexical
/// scope remains the early-return proof: Rust joins this handle before any
/// borrowed input can leave the scope.
...
///
/// # Errors
/// Returns [`ScheduleError`] if a worker panics or shared scheduling state can
...
impl Quiet {
    ...
    ///
    /// # Errors
    /// Returns [`ScheduleError`] when an earlier panic poisoned the shared
    /// scheduling state.
    ...
    ///
    /// # Errors
    /// Returns [`ScheduleError`] when an earlier panic poisoned the exclusive
    /// scheduling state.
