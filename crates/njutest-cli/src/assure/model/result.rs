...
/// Invocation facts to which a raw export must be bound before it can decide
/// anything. They come from the fresh private workspace, not from the export.
...
pub(crate) enum Configuration {
    ...
    Profile,
    /// Caller-supplied compiler flags would make the proof semantics depend
    /// on environment or Cargo configuration outside the retained evidence.
    CompilerFlags,
    /// Caller-supplied compiler selection, wrappers, or bootstrap state could
    /// interpose on the pinned Kani compiler named by the evidence.
    CompilerEnvironment,
    /// The artifact or target directory was not an absolute path.
    RelativePath,
    /// The artifact parent or verifier working directory was absent.
    Directory,
    /// A measured test wrote into the subject tree, so its post-measurement
    /// bytes are not the pristine tree from which the mutation was catalogued.
    TreeWritten,
...
///
/// Malformed input is a value, never an error path a caller can accidentally
/// discard. The only two affirmative decisions are constructed below from
/// the tagged property and all of its sibling properties.
...
/// Compare paths at a protocol boundary without treating macOS's public
/// `/var` spelling and its canonical `/private/var` spelling as different
/// workspaces. Exact clean paths remain comparable in parser-only tests where
/// the retained tree is intentionally absent; an existing path is compared by
...
/// Require `child` to be a strict descendant of `parent`, resolving existing
/// symlinks before the containment decision. The lexical fallback is only for
/// absent retained artifacts and accepts no `.` or `..` component.
...
/// Constructs a fail-closed answer from bytes that never reached the JSON
/// decision protocol (for example a timeout or missing artifact).
