...
//! The `verified-v1` survivor phase: a pristine-tree check followed by one
//! fresh dependency-free proof crate per closed harness.
...
/// An internal inconsistency or I/O failure that prevents the phase itself
/// from asking all of its typed questions.
...
pub(crate) enum ModelError {
    ...
    EmptyPreparation,
    /// The preparation ledger does not name exactly the builds the completed
    /// lattice retained, in the same order.
...
///
/// The contract is a variant rather than a boolean or optional seed, so a
/// caller cannot accidentally treat a non-model run as an empty verified run.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum Preparation {
    /// This contract has no model phase.
    NotRequired,
    /// `verified-v1` retained immutable survivor inputs for later lattice
    /// reconciliation.
    Verified(Seed),
}

/// Immutable model inputs captured by one configured build before its
/// prepared session is closed.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// The exact, cross-build survivor set that may be proved once under the final
/// report namespace.
#[derive(Debug)]
...
impl Preparation {
    ...
    /// Replaces the empty verified seed with the survivors and pristine bytes
    /// captured by this build. Non-model contracts remain structurally inert.
...
impl Plan {
    /// Re-derives one canonical proof plan from the final survivor set and
    /// every configured build's immutable preparation.
    ...
    /// Consumes the plan into the only three inputs the isolated proof phase
    /// can observe.
...
/// Proves that a non-model contract did not accidentally carry a verified
/// preparation through the build lattice.
...
/// Everything the independent model phase needs to construct fresh isolated
/// proof crates. Subject package manifests, build scripts, and dependencies
/// are deliberately absent from this type and therefore cannot enter Kani.
#[derive(Debug, Clone)]
...
/// Collects every test survivor without silently dropping a catalog or source
/// inconsistency.
pub(crate) fn asked(
    session: &rust_mutants::session::Session,
    judged: &[crate::assure::mutation::Judged],
...
/// Generates every closed harness, executes the eligible ones in a clean
/// workspace, and retains one typed record for every survivor offered.
pub(crate) fn prove(
    proving: &Proving<'_>,
    asked: Vec<Asked>,
