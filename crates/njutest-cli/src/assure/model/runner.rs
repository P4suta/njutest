...
//!
//! A process exit is never a proof by itself. This layer pins the tool banner,
//! refuses stale output paths, supervises the whole process tree, retains the
//! raw JSON, and then combines the independently parsed property document with
//! the exact Kani 0.68 exit convention.
...
pub(crate) struct Invocation<'a> {
    /// The absolute `cargo-kani` executable, resolved before subject Cargo
    /// aliases can participate in command selection.
    pub kani: &'a Path,
...
/// Constructs the same retained attempt shape when a pre-execution
/// configuration fact makes invoking Kani unsound.
