...
//!
//! A Rust field type proves a local value.  A private completed type plus a
//! checked constructor can also prove arithmetic and relations across all of
//! its fields once, then preserve that proof because callers cannot mutate
//! the fields.  This module performs that construction-time proof and gives
//! independent audit tools the same explicit invariants to re-derive.
//!
//! Refusals move out of here only when a narrower constructor or sum type
//! makes the invalid relation unconstructible.  They are never dropped merely
//! because the producer currently happens to emit matching values.
...
/// Rechecks a completed report against the conclusion already derived by its
/// checked constructor. Keeping this seam private to the report layer makes
/// the successful derivation part of construction instead of a discarded
/// preflight result.
...
///
/// This is the checked-constructor boundary for [`LatticedReport`]; model
/// correlation is intentionally deferred to the consuming completion
/// transition and then rechecked by [`validate_for_persistence`].
#[must_use]
...
/// Validates the non-empty, ordered build ledger and audits every build as an
/// ordinary report. Cross-build projection is checked only after every source
/// row has independently passed the same audit.
...
///
/// Only `verified-v1` admits model evidence. Under that contract every
/// survivor and every affirmative model outcome has exactly one retained
/// answer, and every retained answer points back to exactly one matching row.
#[derive(Clone, Copy)]
struct ModelAudit<'a> {
