...
/// so empty, padded, or control-bearing names cannot enter a completed
/// evidence ledger and later acquire a second interpretation.
...
/// Why an evidence ledger could not be represented by the report's exact
/// counters.
...
impl TargetAccounting {
    ...
    ///
    /// # Errors
    /// Returns [`CountError`] instead of fabricating a terminal count when
    /// contradictory input exceeds the wire range.
...
pub struct MutantAccounting {
    ...
    pub survived: u32,
    /// How many reached the configured deterministic step boundary without a
    /// control comparison proving that the mutation caused the divergence.
...
pub enum Decision {
    ...
    /// Nothing ran at all.
    Unreached,
    /// The configured step boundary was reached. This is an execution fact,
...
pub enum Outcome {
    ...
    ModelProved,
    /// The configured step boundary was reached. Without a matched control it
    /// establishes neither detection nor survival.
    StepLimitReached,
...
impl Outcome {
    ...
    /// Neither may become detection credit without a separately represented
    /// comparison proving that the mutant diverged from its control.
    #[must_use]
    ...
    ///
    /// This is the single domain boundary used by producers and durable
    /// audits. In particular, an expired clock or a crossed step allowance is
    /// an observation that still needs an answer; it cannot be converted into
    /// one by accepting it.
    #[must_use]
    ...
    ///
    /// `accepted` belongs to the row rather than to an aggregate count. That
    /// prevents an acceptance on one mutation from hiding an unanswered
    /// survivor or unreached mutation elsewhere in the report.
    #[must_use]
    ...
    /// The actionable finding this row must carry, after any row-local
...
pub enum Decided {
    ...
    ModelProved,
    /// The verified runtime guard crossed its configured boundary while this
    /// target ran. This is deliberately a non-verdict.
    StepLimitReached {
...
pub enum Blind {
    ...
    /// Nothing ran it.
    Unreached,
    /// The deterministic guard boundary was reached, without a control proof
    /// that the mutation caused divergence.
    StepLimitReached,
...
///
/// The shard fields are private and can only be constructed after proving
/// `1 <= index <= of`, so an invalid shard cannot enter a completed report.
...
/// Every fact established by one source run for one configured build and one
/// whole catalog or shard.
///
/// Containment retains the namespace that owns every artifact and prevents
/// shard merge from relabelling foreign evidence.
...
impl BuildPartEvidence {
    /// Closes one mutable build draft into source-owned evidence and proves
    /// every row/count/part relation before it can enter a ledger.
...
/// A complete part set: exactly one whole-catalog source, or every shard of
/// one denominator in index order.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub enum PartLedgerError {
    ...
    /// A source timestamp was absent, malformed, or not in the producer's
    /// canonical RFC 3339 spelling.
...
impl PartLedger {
    ...
    ///
    /// # Errors
    /// Returns the first emptiness, shard topology, namespace, catalog, or
    /// accounting invariant violated by `parts`.
    ...
    /// The source whose baseline facts stand for the constructor-proved equal
    /// baseline shared by every shard.
    #[must_use]
...
///
/// The decision is a nested closed object, so direct deserialization and the
/// published schema both reject extra or ambiguous fields.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
pub struct ObserverAccounting {
    ...
    pub model_proved: u32,
    /// How many crossed the configured step boundary without a mutation/control
    /// comparison establishing a verdict.
    pub step_limit_reached: u32,
...
impl ObserverAccounting {
    ...
    ///
    /// # Errors
    /// Returns [`CountError`] when mutually exclusive columns cannot be
    /// represented by the report's total.
...
/// A closed construction failure for the compiler-checked model evidence
/// graph. These are programmer/protocol invariants, never user prose.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
...
/// The only retained model-artifact namespace: one file named by the full
/// mutation identity under the run's fresh `model` directory.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(transparent)]
...
/// The complete identity of the dependency-free crate handed to
/// Kani. Fields are private and deserialization is checked so a report cannot
/// claim a different package, edition, source path, network policy, or lock
/// policy while retaining an affirmative model answer.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// Producer-side ingredients which become evidence only after all relational
/// identity invariants have been checked.
...
///
/// Its fields are private because the raw artifact, its digest, and the
/// optional verifier are one fact: a complete artifact must carry its own
/// digest, no artifact must carry the digest of the empty sequence, and a
/// verifier identity cannot exist without the export that established it.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// Producer-side attempt facts which become evidence only after their
/// cross-field invariants have been checked.
...
pub enum ModelConfiguration {
    ...
    Profile,
    /// Caller or Cargo configuration supplied compiler flags outside the
    /// retained proof identity.
    CompilerFlags,
    /// Caller configuration selected or interposed on the compiler named by
    /// the pinned Kani evidence.
    CompilerEnvironment,
...
///
/// This is deliberately nested beneath the `undecided` decision. Keeping the
/// reason beside an independently constructible evidence product would admit
/// nonsense such as `cutoff` with a normal exit or `tool/unavailable` after a
/// verifier identity had been established.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
///
/// This is not a presentation counter.  Shard membership is a function of
/// this value, so retaining it is what lets a durable reader prove that a row
/// was not moved between parts after execution.
...
/// The exact ordered survivor set a completed lattice offers to the model
/// phase.  The owner is the final report namespace, not any per-build source
/// namespace.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ModelCandidates {
    /// Every configured build, in the exact order proved by the completed
    /// build ledger.
...
///
/// Construction compares the complete record sequence with a
/// [`ModelCandidates`] value minted by the same [`LatticedReport`].  A caller
/// therefore cannot silently omit, duplicate, reorder, or add a model answer.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
...
///
/// The sum type distinguishes a contract that has no model phase from a
/// verified contract whose exact survivor set has been answered.  There is no
/// `None`/pending state in a serializable [`Report`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(
    tag = "kind",
    content = "batch",
...
///
/// The decision and reuse provenance are nested closed objects, so direct
/// deserialization and the published schema both reject extra or ambiguous
/// fields.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct MutantRecord {
    /// The dense canonical catalog position used to prove ordering and shard
    /// membership.
    ...
    ///
    /// Kept on the row so durable accounting can be re-derived rather than
    /// trusted as an independent counter.
    pub accepted: bool,
...
///
/// Producers, shard merging, configured-build reconciliation, and the
/// persistence audit all call this function. A new outcome therefore cannot
/// acquire four subtly different accounting rules.
...
pub enum FindingKind {
    ...
    WaitedMutant,
    /// A mutation crossed its verified step allowance without a matched
    /// control establishing divergence.
...
pub enum FindingOrigin {
    ...
    Global,
    /// Evidence produced by one retained source namespace for one configured
    /// build and catalog part.
    Source {
...
///
/// Both spellings have already passed the canonical timestamp parser at the
...
///
/// Wall-clock span and summed compute time are different quantities and are
/// deliberately named separately rather than storing one ambiguous
/// `duration_ms`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
...
///
/// Target and mutation projections span the configured request.  Soundness
/// facts remain build-qualified because taking a maximum would erase which
/// program was actually inspected.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
...
///
/// This type is deliberately not serializable. Completion consumes one or
/// more build reports into [`Report`], whose non-empty build ledger is the
...
impl BuildReport {
    ...
    ///
    /// # Errors
    /// Returns [`CountError`] when the durable counters cannot represent all
    /// retained rows.
...
///
/// The default build is stored separately from the remaining builds, so an
/// empty completed report cannot be represented. Construction also fixes the
/// catalog relation once; callers can inspect but never mutate the entries.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub enum BuildLedgerError {
    ...
    DurationOverflow,
    /// Constructor-proved source timing could not be re-read while deriving
    /// the complete ledger span.
...
///
/// Unlike the projection, this remains bound to the source namespace and
/// catalog part that established it.  Acceptance, routing, and reuse never
/// leak into a synthetic representative row.
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// This deliberately has no synthetic `accepted`, `routing`, `reuse`, or
/// selected source outcome.  Those facts remain on [`BuildMutationDecision`]
/// entries, while `decision` is only the lattice minimum across them.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl MergeSources {
    /// Canonicalizes unordered inputs by shard index and proves exact
    /// denominator coverage plus namespace uniqueness.
...
///
/// This value is deliberately not serializable.  It can become a durable
/// [`Report`] only by taking one of the contract-checked consuming transitions:
/// [`LatticedReport::complete_without_models`] or
/// [`LatticedReport::attach_models`].
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// derived verdicts, counters, and row projections are never stored or
/// serialized. Unlike [`LatticedReport`], this type always carries the exact
/// terminal model state required by its contract.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct Report {
    /// [`SCHEMA`].
    ...
    builds: BuildLedger,
    /// Run-wide findings derived without executing any one build, retained
    /// exactly once rather than copied into each source.
    global_findings: Vec<Finding>,
    /// The terminal contract-specific model state produced after the
    /// cross-build and cross-shard lattice.
...
pub enum CompletionError {
    ...
    /// A constructor-proved catalog row could not be recovered as its typed
    /// mutation identity while minting the model candidate set.
...
impl LatticedReport {
    ...
    /// The assurance contract that determines the only legal completion
    /// transition.
    #[must_use]
    ...
    /// The exact ordered survivor set to pass to the one post-lattice model
    /// phase.
    ///
    /// # Errors
    ...
    ///
    /// # Errors
    /// `verified-v1` cannot take this transition; it must call
    /// [`Self::attach_models`].
    ...
    /// Attaches the exact final-run-owned model batch and completes
    /// `verified-v1`.
    ///
    /// # Errors
    /// Refuses another contract, another owner, or any record sequence other
    /// than the exact canonical survivor sequence.
...
impl Report {
    ...
    /// Reissues an immutable completed answer under a new response run while
    /// retaining every evidence namespace and measurement time that actually
    /// established it.
    ///
    /// # Errors
    /// Refuses model-complete evidence because its retained artifacts are
    /// owned by the original final-run namespace.
    pub fn read_back_as(
        mut self,
        run_id: &rust_mutants::id::RunId,
    ) -> Result<Self, CompletionError> {
...
/// One deliberately partial `K/N` document.  It cannot be passed where a
/// completed [`Report`] is required; only shard merge consumes it.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
...
/// Why one mutable shard measurement could not become an immutable shard
/// document.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
...
///
/// Whole-catalog evidence remains non-durable until it takes the required
/// model transition.  A shard is already terminal partial evidence and has no
/// model API at all.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// The strict durable assurance-document boundary.  Complete and partial
/// evidence are distinct variants and cannot be confused by a caller.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(
    tag = "document_type",
    content = "report",
...
impl ReportDocument {
    /// The final envelope namespace, distinct from all retained evidence
    /// source namespaces.
    #[must_use]
    pub fn run_id(&self) -> &str {
        match self {
