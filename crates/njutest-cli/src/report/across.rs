...
///
/// The primary `default` build is stored separately, so reconciliation cannot
/// be called with no evidence and invent an error decision for an empty set.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl BuildMeasurements {
    ...
    ///
    /// # Errors
    /// Returns the exact name, ordering, emptiness, or uniqueness invariant
    /// violated by the supplied build measurements.
    pub fn checked(
