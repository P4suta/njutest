...
//!
//! The value only. What a person reads is a separate job, and the line
//! between them is that this knows nothing about how it is shown and a page
//! invents nothing this does not carry.
...
///
/// Both halves of the product mint sixty-four hex characters in separate
/// identity domains, so the name a person types does not say on its own which
/// was meant. Asking is how the caller says, and answering about the other
/// one would be answering a question nobody put.
...
///
/// Three answers rather than two, and the third is the reason this is an
/// enum. A run that kept no recording establishes nothing about why anything
/// happened; a recording that does not name the claim establishes that the
/// claim is not in it. Telling a reader "nothing stands behind this" about
/// the first would be this command concluding from how it was measured —
/// the defect it exists to explain, arriving in the explanation.
///
/// The two are acted on differently. One is a missing `--trace=`. The other
...
///
/// The claim and its decision travel in one value: a mutation cannot come to
/// a seam's decision, and three fields where two of them can disagree is the
...
///
/// A closed set with no timestamps in it. *Why did this come out this way*
/// and *why did this take four minutes* are two questions, and `njutest
/// trace` owns the second — it names the slowest work, which is what
/// somebody making a run shorter is after. A page carrying durations would
/// grow a column nobody reads on every chain where each step is instant.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Step {
...
/// the caller says which it has by handing over `None`, because a reader of
/// this value cannot tell an absent file from an empty one and would be told
