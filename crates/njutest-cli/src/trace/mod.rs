...
/// A source of the current moment: the recorder's one test seam, so a test can
...
/// Production has exactly one clock. Keeping that state private means the
/// deterministic injection seam does not become part of an installed CLI's
/// library surface.
...
impl Clock {
    ...
    /// The next moment, or the exact representational failure that made the
    /// deterministic clock unusable.
    #[cfg(any(test, feature = "testkit"))]
...
impl Recorder {
    ...
    /// Starts a recording into `sink` with an injected test clock and emits
    /// its `run-start` event.
    #[cfg(any(test, feature = "testkit"))]
    #[must_use]
    #[cfg(feature = "testkit")]
