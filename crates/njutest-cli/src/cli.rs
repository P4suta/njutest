...
pub struct Verify {
    ...
    #[arg(long, value_name = "DIR")]
    pub directory: Option<PathBuf>,
    /// The configuration file. `.njutest.toml` beside the workspace by default; a missing one is the defaults.
    ...
    /// Judge only one part of the catalog, as `K/N`. Every part measures the whole baseline; `njutest merge` combines what they judged.
    #[arg(long, value_name = "K/N")]
    pub shard: Option<String>,
    /// Arguments for the test binaries. Only the flags njutest does not own are allowed: --test-threads, --include-ignored, --nocapture, --show-output.
