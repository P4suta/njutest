...
///
/// Every name a reader might want to go and look at is on the line that
/// mentions it. A step saying a proof removed a target without naming the
/// proof is a verb with no agent, and the reader is told something was
/// removed and not what removed it, which is the one thing they would check.
...
///
/// A run that kept no recording and a recording that does not name the claim
/// are two pages, never one with a number in it. The first establishes
/// nothing about this claim or any other, and telling a reader their identity
/// is not in the recording would be this command concluding from how it was
/// measured — the defect it exists to explain, arriving in the explanation
/// (ADR 0023). The second is a name typed wrong, and the count of what the
/// recording does hold is how a reader tells that from a run that recorded
/// nothing of the kind.
///
/// No step carries a duration. *Why did this come out this way* and *why did
/// this take four minutes* are two questions and `njutest trace` owns the
/// second; a column blank on every chain where each step is instant is a
/// column nobody reads.
#[must_use]
