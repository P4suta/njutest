...
///
/// Keyed on the locator without its line and on what the run replaced, so two
/// gaps of one rule on one item that differ only by line are one entry. That
/// is the trade: a watcher is reading for change, and the second of two
/// identical gaps was never the news.
...
///
/// Only gaps in the tests are compared. A measurement the run could not
/// settle is not a gap that the reader closed by editing — it is a run that
/// established nothing, twice, and counting its disappearance as progress is
/// this surface concluding from how it measured (ADR 0023).
#[must_use]
