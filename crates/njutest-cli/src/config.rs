...
///
/// The private UTF-8 spelling is platform independent: forward slashes are
/// the only separators and every component is a non-empty ordinary name.
...
pub struct Execution {
    ...
    /// `step_limit_reached` rather than `waited`. It is a deterministic
    /// execution fact, not a mutation verdict without a matched control.
...
///
/// Both values are optional in the document so serde can distinguish a
/// missing key from a zero one. [`Verification::checked`] is the only route
/// to the invariant-bearing [`Verified`] value consumed by the model runner.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields, default)]
...
impl Verification {
    /// Turns document-shaped optional values into the values a verifier may
    /// actually consume.
    ///
    /// # Errors
    /// Returns the first missing or zero bound. No caller can accidentally
    /// turn either condition into an unbounded proof.
...
impl Config {
    ...
    ///
    /// # Errors
    /// Returns a typed error when a `verified-v1` bound is absent or zero, or
    /// when another contract carries verifier-only keys. This rechecks public
    /// fields so a caller that constructs or mutates [`Config`] cannot bypass
    /// the same boundary enforced by [`Config::parse`].
...
impl Config {
    ...
    ///
    /// # Errors
    /// [`ConfigErrorKind::Invalid`] when the validated in-memory value cannot
    /// be serialized. No placeholder document is substituted into identity.
