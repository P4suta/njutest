...
//! `mjutest lsp`: what a completed run found, in the editor the code is being written in.
//!
...
fn capabilities(encoding: Encoding) -> Value {
    json!({
        ...
        "serverInfo": { "name": "mjutest", "version": crate::VERSION },
    })
}
...
/// `mjutest accept` is what writes an acceptance, so handing back what to type
...
fn actions(request: &Value) -> Value {
    ...
    for diagnostic in held {
        ...
        offered.push(json!({
            "title": format!("mjutest accept {mutant} --reason '…'"),
            "kind": "quickfix",
            "diagnostics": [diagnostic],
            "command": {
                "title": "Copy the acceptance to record",
                "command": "mjutest.accept",
                "arguments": [mutant],
            },
        }));
    }
