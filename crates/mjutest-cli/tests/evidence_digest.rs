...
use mjutest_cli::evidence::digest::{EVIDENCE_DOMAIN, Fields, Inputs, Mode, identity};
...
fn a_field_list_is_length_prefixed_so_two_different_lists_never_collide() {
    ...
    let mut domain = Fields::new("mjutest-something-else-v1");
    domain.field("tree", "x");
...
fn a_run_that_judged_one_part_of_a_catalog_is_not_the_run_that_judged_all_of_it() {
    ...
    assert_eq!(EVIDENCE_DOMAIN, "mjutest-evidence-v3");
}
