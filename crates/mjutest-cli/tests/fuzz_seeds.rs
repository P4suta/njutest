...
const READERS: [(&str, Reader); 15] = [
    ...
    ("carried_answers", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
            text.lines()
                ...
                .all(|line| mjutest_cli::report::json::parse(line).is_ok())
        })
    }),
    ("config", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
            mjutest_cli::config::Config::parse(text, Path::new(".mjutest.toml")).is_ok()
        })
    }),
    ("coverage_export", |data| {
        mjutest_cli::coverage::parse_export(data).is_ok_and(|files| !files.is_empty())
    }),
    ("depinfo", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
    ...
    ("offered_candidates", |data| {
        std::str::from_utf8(data).is_ok_and(|text| {
            mjutest_cli::repair::take(text, Path::new("/w"), &mjutest_cli::repair::allowed(&[]))
                .is_ok()
        })
    }),
    ("report_document", |data| {
        std::str::from_utf8(data).is_ok_and(|text| mjutest_cli::report::json::parse(text).is_ok())
    }),
    ("touch_log", |data| {
        std::str::from_utf8(data)
...
fn seeds() -> PathBuf {
    mjutest_devkit::paths::workspace_root().join("fuzz/seeds")
}
