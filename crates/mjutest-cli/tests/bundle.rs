...
use mjutest_cli::app::diagnostics::{MANIFEST_NAME, copy, copy_tree, record, run};
use mjutest_cli::app::reports::{DOCUMENT_NAME, RUNS_DIR};
use mjutest_cli::cli::{Diagnostics, Environment};
...
fn a_bundle_with_no_optional_evidence_names_every_absence_exactly() {
    ...
    let bundle = dir.path().join(".mjutest/diagnostics").join(RUN);
    assert_eq!(
        stdout,
        format!(
            ...
            mjutest_cli::trace::FILE_NAME,
            mjutest_cli::trace::OUTPUT_DIRECTORY_NAME,
        ),
    ...
    assert_eq!(value["schema"], "mjutest-diagnostics-v1");
...
fn bundle_directory_and_manifest_failures_are_errors_with_no_success_output() {
    ...
    std::fs::create_dir_all(blocked.path().join(".mjutest")).expect("state directory");
    std::fs::write(
        blocked.path().join(".mjutest/diagnostics"),
        "not a directory\n",
    )
    ...
    let bundle = unwritable.path().join(".mjutest/diagnostics").join(RUN);
