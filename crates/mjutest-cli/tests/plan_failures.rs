...
fn asked(environment: &Environment, extra: &[&str]) -> Said {
    let mut args = vec!["mjutest", "plan", "--offline", "--locked"];
...
fn an_invalid_configuration_is_an_error_and_not_a_panic() {
    let repo = project();
    repo.write(".mjutest.toml", "version = 1\nunknown = true\n");
    ...
    assert!(said.err.contains("MJ1002"), "{}", said.err);
}
...
fn unreadable_build_output_is_an_error_and_not_a_panic() {
    ...
    assert!(said.err.contains("MJ3003"), "{}", said.err);
...
fn an_unusable_scratch_parent_is_an_error() {
    ...
    assert!(said.err.contains("MJ8001"), "{}", said.err);
}
...
fn a_plan_builds_in_its_dedicated_scratch_target_and_raii_removes_it() {
    ...
    assert!(
        scratch
            ...
            .is_some_and(|name| name.starts_with(mjutest_cli::scratch::DIR_PREFIX)),
        "{}",
        scratch.display()
    );
    assert!(
...
fn target_enumeration_failure_is_an_error_and_not_a_panic() {
    ...
    assert!(said.err.contains("MJ3001"), "{}", said.err);
...
fn build_defaults_command_environment_limit_and_trace_are_exact() {
    ...
    assert_eq!(
        ...
        Some(
            scratch
                .join(mjutest_cli::build::BUILD_PROFILES)
                .join("%p-%m.profraw")
                .display()
                .to_string()
        )
