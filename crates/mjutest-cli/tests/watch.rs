...
fn watching(arguments: &[&str], environment: &Environment) -> (u8, String, String) {
    ...
    let args = std::iter::once("mjutest")
        .chain(arguments.iter().copied())
        .map(OsString::from);
    let code = mjutest_cli::run_from(args, environment, &mut out, &mut err);
    (
        code,
...
fn a_configuration_that_cannot_be_read_ends_the_watch_before_it_looks() {
    let root = tempfile::tempdir().expect("a directory");
    std::fs::write(root.path().join(".mjutest.toml"), "[project\n").expect("a configuration");
    let environment = stopped(root.path());
    ...
    assert!(
        complained.contains(".mjutest.toml"),
