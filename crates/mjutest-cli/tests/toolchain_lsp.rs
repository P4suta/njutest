...
//! The language server on the process's own streams: what an editor that starts `mjutest lsp` gets back.
...
use mjutest_cli::app::lsp::{framed, message};
use mjutest_cli::config::Contract;
use mjutest_cli::report::{Finding, FindingKind, MutantRecord, Position, Report, RunKind};
