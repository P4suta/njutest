...
use mjutest_cli::assure::equivalence::{Decided, settle};
use mjutest_cli::assure::mutation::{Disposition, Judged};
use mjutest_cli::config::Contract;
use mjutest_cli::evidence::{digest, key};
...
fn environment(count: u32) -> Vec<(String, String)> {
    (0..count)
        .map(|index| (format!("MJUTEST_VAR_{index}"), format!("value-{index}")))
        .collect()
}
...
fn benchmarks(criterion: &mut Criterion) {
    ...
    let watch = mjutest_cli::watch::Watch::new(&cancel, &trace);
