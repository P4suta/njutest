...
fn measured(name: &str, status: TargetStatus) -> Measured {
    let unit = njutest::targets::UnitKind::Lib;
    Measured {
        target: njutest::targets::Target {
            id: njutest::targets::target_id("core", unit, "core", name)
...
fn what_a_run_is_about_is_what_it_was_asked_for_narrowed_to_what_is_there() {
    ...
    assert_eq!(
        njutest::assure::run::unknown_package(&named, &["core".to_owned()]),
        Some("app".to_owned()),
...
fn how_much_of_the_workspace_a_run_looked_at_is_part_of_what_it_may_conclude() {
    ...
    changed.changed = Some(njutest::git::Change {
        base: "main".to_owned(),
        merge_base: None,
...
fn resource(exclusive: bool) -> njutest::config::Resource {
...
fn a_tree_the_compiler_vouches_for_entirely_has_no_limitation_to_state() {
    assert_eq!(
        stated(&njutest::soundness::Inventory::default()),
        Vec::new(),
    ...
    let found = njutest::soundness::Inventory {
        items: vec![njutest::soundness::Item {
            package: "core".to_owned(),
            path: "core/src/lib.rs".to_owned(),
            line: 7,
            kind: njutest::soundness::Kind::Block,
        }],
    ...
    let unread = njutest::soundness::Inventory {
...
fn a_run_that_looked_at_less_than_everything_neither_believes_nor_records() {
    ...
    let resources: std::collections::BTreeMap<String, njutest::config::Resource> =
...
fn planned(unit: njutest::targets::UnitKind, tests: usize, ignored: usize) -> Planned {
    Planned {
        target: njutest::targets::Target {
            id: njutest::targets::target_id("demo", unit, "demo", "whole binary")
                .expect("the fixture target identity"),
...
fn a_plan_names_a_binary_and_says_what_put_it_there_only_when_asked() {
    let ordinary = planned(njutest::targets::UnitKind::Lib, 7, 2);
    assert_eq!(
        line(&ordinary, false),
    ...
    assert!(
        line(&planned(njutest::targets::UnitKind::Doc, 3, 0), true)
    ...
    assert!(
        line(&planned(njutest::targets::UnitKind::Test, 0, 0), true).contains("answers by exiting"),
    ...
    assert!(
        line(&planned(njutest::targets::UnitKind::Test, 0, 4), true).contains("4 tests, 4"),
