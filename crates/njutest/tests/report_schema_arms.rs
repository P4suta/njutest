...
///
/// Built from the golden rather than from the model on purpose: the model writes whichever disposition the run it is given reaches, so a suite that only ever builds one report only ever shows the schema one of its shapes.
/// A document is bytes, and bytes are what the published contract is about.
///
/// Nothing about the accounting is patched.
/// It was, while the model still wrote a single `timed_out` and the schema already said two; the day the golden caught up, the patching started writing nulls into the two columns it had been adding, and these tests failed for the scaffolding rather than for the shape.
...
fn dispositions() -> Vec<(&'static str, bool)> {
    njutest::report::Decided::every_against("0123456789abcdef")
        .iter()
...
fn only_a_step_limit_record_carries_one_verified_boundary() {
    ...
    mismatched["report"]["builds"][0]["parts"][0]["mutants"][0]["step_boundary"]["observed"] =
        json!(12);
    assert!(
        serde_json::from_value::<njutest::report::Report>(mismatched).is_err(),
