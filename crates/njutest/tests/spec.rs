...
use njutest::report::spec::{page, spoken};
...
fn question(seq: u64, rule: &str, decision: SeamDecision) -> SeamRecord {
    SeamRecord {
        ...
        answered: Some(201),
        rule: njutest::wire::rule::Rule::parse(rule)
            .unwrap_or(njutest::wire::rule::Rule::DropConnection),
        decision,
    }
}
...
fn a_sentence_nothing_would_notice_changing_says_so_rather_than_saying_nothing() {
    ...
    let said = spoken(&report)
        ...
        .first()
        .map(njutest::report::spec::Sentence::worded)
        .expect("a sentence");
    assert!(
