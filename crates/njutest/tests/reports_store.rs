...
fn platform_report() -> Result<njutest::report::Report, PlatformReportError> {
    ...
    let latticed = njutest::report::across::configured(&run_id, &measurements)?;
...
mod unix {
    ...
    use njutest::app::reports::{
        DOCUMENT_NAME, HTML_NAME, Index, JUNIT_NAME, SARIF_NAME, SCHEMA_NAME, StoreError, keep,
        pointed_at, retain,
    };
    use njutest::report::across::{BuildMeasurements, BuildMeasurementsError, ConfiguredError};
    ...
    /// The default fixture report root.
    /// Production deliberately keeps this spelling behind a held [`njutest::app::reports::Store`]; tests that arrange hostile filesystem entries spell the documented default locally instead of reopening a production authority as a raw path.
    ...
    /// Consumes mutable source evidence through the same checked lattice and contract transition as production before a store can observe it.
            ...
            (
                |root| {
                    run_path(root, &stored_run_id(BLOCKED_RUN))
                        .join(njutest::report::lines::FILE_NAME)
                },
                false,
                None,
            ),
        ...
        for (of, file_in_place_of_directory, derived_index) in blocked {
            ...
            if let Some(expected) = derived_index {
                ...
                assert!(
                    matches!(
                        kept.indexes,
                        njutest::app::reports::IndexPublication::Incomplete { index, .. }
                            if index == expected
                    ),
