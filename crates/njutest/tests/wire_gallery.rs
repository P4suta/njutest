...
use njutest::report::{BuildReport, Report, RunKind, SeamDecision, SeamRecord};
use njutest::wire::derive::{Fault, derive};
...
fn every_sentence_a_watched_seam_can_say_is_one_somebody_has_looked_at() {
    ...
    let mut out = String::from(
        "Every sentence a watched seam puts in front of a person.\n\
         Written by crates/njutest/tests/wire_gallery.rs; UPDATE_GOLDEN=1 rewrites it.\n",
    );
    for (name, decide) in cases {
        ...
        out.push_str(
            &njutest::report::spec::page(&report).expect("the gallery sentences are countable"),
        );
