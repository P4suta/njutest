...
use njutest::wire::interpose::{Interposer, Interposing};
use njutest::wire::rule::Rule;
use njutest::wire::{Spoken, Wire};
...
///
/// It is told what to say and never how often.
/// A helper that stops at a number stops answering the moment the number is wrong, and a caller left without an answer blocks,
/// so the test hangs until something outside it gives up and there is nothing to read.
/// The number is not a thing a test can know, either: a retry, a keep-alive, or a client that opens two connections all move it without moving what is being measured.
/// How many there were is something to assert afterwards, by asking `served`.
struct Upstream {
...
fn injecting(
    ...
    fault: Option<njutest::wire::derive::Fault>,
) -> Interposer {
...
fn fault(rule: &str, seq: u64) -> njutest::wire::derive::Fault {
    njutest::wire::derive::Fault {
        id: "f".repeat(64),
        capability: "api".to_owned(),
        seq,
        during: None,
...
fn lease(capability: &str, named: &[(&str, &str)]) -> njutest::resource::Lease {
    njutest::resource::Lease {
...
fn a_seam_the_configuration_names_is_one_the_tests_dial_through() {
    ...
    let Ok(watching) = njutest::wire::dialled::interposed(&held, "BASE_URL", (Wire::Http, BRIEFLY))
    else {
...
fn a_lease_with_no_such_variable_is_left_as_the_provider_gave_it() {
    ...
    let Err(refused) = njutest::wire::dialled::interposed(&held, "BASE_URL", (Wire::Http, BRIEFLY))
    else {
        panic!("a lease carrying no such variable is one nothing can be put in front of")
    };
    assert_eq!(
        refused,
        njutest::wire::dialled::NotWatched::NoSuchVariable,
...
fn a_seam_whose_authority_names_a_host_is_watched_like_any_other() {
    ...
    let watching = njutest::wire::dialled::interposed(&held, "BASE_URL", (Wire::Http, BRIEFLY));
    assert!(
        watching.is_ok(),
...
fn a_seam_the_run_could_not_watch_says_which_of_the_ways_it_could_not() {
    ...
    let refused = njutest::wire::dialled::interposed(&held, "BASE_URL", (Wire::Http, BRIEFLY))
        .expect_err("a value naming no authority");
    assert_eq!(
        refused,
        njutest::wire::dialled::NotWatched::NamesNoAuthority
    );
    assert!(
...
fn ask_url(url: &str) -> String {
    let authority = njutest::wire::dialled::upstream_of(url).expect("an authority");
...
fn only_the_seams_the_configuration_names_are_watched() {
    use njutest::assure::wire::watched;
...
fn seam(capability: &str, upstream_at: std::net::SocketAddr) -> njutest::wire::dialled::Watching {
    njutest::wire::dialled::Watching {
        capability: capability.to_owned(),
        environment: Vec::new(),
...
fn every_question_a_seam_licensed_is_put_to_the_suite_one_at_a_time() {
    ...
    let done = njutest::assure::wire::asking(
        ...
        || {
            ...
            njutest::wire::settle::Asked::Answered(vec![njutest::wire::settle::Answered {
                target: "pkg/test/it".to_owned(),
                passed: true,
            }])
        },
...
fn a_question_the_seam_never_reached_is_reported_as_a_hole_and_never_as_a_survivor() {
    ...
    assert!(
            ...
            .iter()
            .all(|one| one.kind == njutest::report::FindingKind::NotMeasured),
