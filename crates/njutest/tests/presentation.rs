...
fn width(text: &str) -> usize {
    njutest::presentation::wide(text)
}
...
fn what_to_draw_for_is_decided_from_what_was_found_out_and_nothing_else() {
    use njutest::presentation::{Asked, Glyphs, ROOM, Reader, Wanted};
...
fn the_label_under_a_mark_says_what_the_run_established_and_never_something_else() {
    ...
    let said = |decided: Decided| njutest::presentation::label("gt-to-ge", &decided);
...
fn every_outcome_a_record_can_hold_reads_differently_from_the_others() {
    let said: Vec<String> = njutest::report::Decided::every_against("pkg/test/it")
        .iter()
        .map(|decided| njutest::presentation::label("gt-to-ge@1", decided))
        .collect();
...
fn an_outcome_that_happened_to_a_target_names_that_target_and_not_another() {
    for decided in njutest::report::Decided::every() {
        let Some(against) = decided.decided_by() else {
            continue;
        };
        let said = njutest::presentation::label("gt-to-ge@1", &decided);
        assert!(
