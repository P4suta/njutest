...
use njutest::wire::derive::derive;
use njutest::wire::prove::{ALREADY_THAT_ANSWER, NO_BODY_TO_CUT, discharges};
...
fn answered(body_bytes: u64) -> Exchange {
    said(body_bytes, 204, "HTTP/1.1 204 No Content")
}

/// One answer of `body_bytes` bytes, opening with `status_line`.
fn said(body_bytes: u64, status: u16, status_line: &str) -> Exchange {
    Exchange {
...
fn asking(observed: &[Exchange], rule: &str) -> Option<njutest::wire::derive::Fault> {
...
#[test]
fn asking_for_the_answer_the_upstream_already_gave_is_a_question_nobody_could_answer() {
    let observed = vec![said(9, 500, "HTTP/1.1 500 Internal Server Error")];
    assert_eq!(
        discharges(
            &asking(&observed, "status-server-error").expect("the question"),
            &observed
        ),
        Some(ALREADY_THAT_ANSWER),
        "the run would write the line that is already there, so the caller is handed \
         the bytes it was handed. The check is asked of the function that does the \
         restating, which is what makes it a check on the injection rather than a \
         second opinion about it"
    );
}

#[test]
fn an_upstream_that_worded_the_same_code_differently_is_still_asked() {
    let observed = vec![said(9, 500, "HTTP/1.1 500 Server Error")];
    assert_eq!(
        discharges(
            &asking(&observed, "status-server-error").expect("the question"),
            &observed
        ...
        None,
        "a 500 whose reason phrase differs from the one a run writes is a different \
         answer on the wire, and a proof that held only most of the time would not be \
         a proof"
    );
}

#[test]
fn asking_for_a_different_code_than_the_one_given_is_still_asked() {
    let observed = vec![said(9, 500, "HTTP/1.1 500 Internal Server Error")];
    assert_eq!(
        discharges(
            &asking(&observed, "status-not-found").expect("the question"),
            &observed
        ),
        None,
        "404 is not what this upstream said, so the caller would be handed something \
         it was not handed"
    );
}
...
fn nothing_is_proved_about_an_exchange_the_run_did_not_observe() {
    let observed = vec![answered(0)];
    let elsewhere = njutest::wire::derive::Fault {
