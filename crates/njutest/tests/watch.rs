...
use njutest::app::watch::{POLL, Seen, look, until};
use njutest::cli::{EXIT_ERROR, Environment};
use njutest::testkit::watch_until_with_wait;
...
fn watching(arguments: &[&str], environment: &Environment) -> (u8, String, String) {
    ...
    let args = std::iter::once("njutest")
        ...
        .map(OsString::from);
    let code = njutest::run_from(args, environment, &mut out, &mut err);
    (
        code,
...
fn excluded() -> njutest::evidence::tree::Excluded {
...
fn looking_reads_every_file_under_verification_and_none_of_what_a_run_writes() {
    ...
    let reports = njutest::config::Config::default().reports.directory;
