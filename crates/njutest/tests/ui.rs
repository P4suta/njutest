...
use njutest::cli::Ui;
use njutest::ui::Notes;

fn said(kind: Ui, say: impl FnOnce(&mut Notes<'_>) -> std::io::Result<()>) -> String {
    let mut out = Vec::new();
    {
        let mut notes = Notes::of(kind, &mut out);
        say(&mut notes).expect("the in-memory stream accepts the note");
        notes
            .finish()
            .expect("the in-memory stream accepts finalization");
    }
...
fn the_jsonl_interface_writes_one_object_for_each_thing_that_happened() {
    ...
    let objects: Vec<serde_json::Value> = text
        .lines()
        .map(|line| njutest_devkit::strictjson::decode_str(line).expect("one object per line"))
        .collect();
...
fn the_dashboard_keeps_one_block_and_redraws_it_rather_than_scrolling() {
    ...
    assert_eq!(
        ...
        1,
        "redraws do not scroll, and finalizing the dashboard ends its one line: {text:?}"
    );
    assert!(
