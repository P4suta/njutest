...
fn every_flag_a_refusal_names_is_one_this_release_parses() {
    let mut command = njutest::cli::Request::command();
    command.build();
    let mut known = std::collections::BTreeSet::new();
    accepted(&command, &mut known);
    let mut named = 0_usize;
    let mut missing = Vec::new();
    for code in njutest::error::error_codes() {
        for text in [code.summary, code.remedy] {
            for flag in njutest_devkit::named_flags::named_flags(text, "njutest") {
