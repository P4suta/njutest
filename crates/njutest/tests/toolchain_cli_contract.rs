...
fn init_writes_a_skeleton_that_loads_as_exactly_the_defaults() {
    ...
    let path = dir.path().join(njutest::config::FILE_NAME);
    ...
    assert!(
        njutest_devkit::process::strict_utf8(&output.stdout).contains(njutest::config::FILE_NAME),
...
fn the_word_cargo_repeats_is_dropped_once_and_only_where_it_is_the_subcommand() {
    ...
    let elsewhere = njutest::cli::parse(
