...
use njutest::assure::identity::Evidence;
use njutest::assure::run::{Request, identity, opened};
use njutest::build::Cargo;
use njutest::config::Config;
use njutest::limitation::WORKSPACE_DIGEST_NOT_COMPUTED;
use njutest::report::{RunKind, UNAVAILABLE};
...
fn a_run_that_could_not_ask_git_says_so_before_it_compiles_anything() {
    ...
    request.evidence = known();
    let scratch = njutest::scratch::Scratch::create(
        parent.path(),
    ...
    opened(
        ...
        &request,
        &environment,
        (&scratch, njutest::watch::Watch::new(&cancel, &trace)),
    )
