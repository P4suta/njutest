...
fn an_exchange_read_as_one_protocol_and_carrying_another_is_not_read_back() {
    let raw = njutest::trace::Read::Raw;
    let http = njutest::trace::Read::Http {
        method: "GET".to_owned(),
    ...
    for (what, written) in [
        ...
        let read: Result<njutest::trace::Read, serde_json::Error> =
...
fn a_recording_opens_with_run_start_and_closes_with_run_end() {
    ...
    assert_eq!(start.njutest, njutest::VERSION);
...
fn a_recording_that_stops_being_readable_is_an_error_and_never_a_shorter_recording() {
    ...
    assert!(
        matches!(stopped, njutest::trace::ReadError::Io { .. }),
...
fn a_blank_line_is_passed_over_and_a_line_that_is_not_an_event_names_itself() {
    ...
    assert!(
        matches!(
            refused,
            njutest::trace::ReadError::Malformed { line: 3, .. }
        ),
...
fn the_v1_reader_distinguishes_an_explicit_null_from_a_missing_field() {
    let event = Event {
        ...
        payload: Payload::PhaseStart {
            phase: njutest::trace::PhaseRecord {
