...
//! Every `njutest` command a documented workflow runs is one this release parses.
...
#[test]
fn every_documented_njutest_command_in_a_workflow_parses() {
    let mut asked = 0_usize;
    let mut refused = Vec::new();
    for (path, text) in pages() {
        for command in njutest_devkit::workflow_commands::commands(&text, "njutest") {
            asked += 1;
            if let Err(error) = njutest::cli::Request::try_parse_from(&command.argv) {
                refused.push(format!(
                    "{}:{}: {}\n{}",
                    path.display(),
                    command.line,
    ...
    assert!(
        asked > 0,
        "the documentation shows no workflow that runs njutest, so this law holds nothing"
    );
    assert!(
        refused.is_empty(),
