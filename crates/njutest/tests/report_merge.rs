...
use njutest::report::StepBoundary;
use njutest::report::merge::{MergeError, merge};
use njutest::report::{
...
///
/// The target is supplied where the outcome needs one, because the pairing is the thing under test everywhere else and a fixture that could not build a valid one would be testing the fixture.
fn decided(outcome: &str, by: Option<&str>) -> njutest::report::Decided {
    ...
    njutest::report::Decided::of(held, Some(named), boundary)
        .or_else(|| njutest::report::Decided::of(held, None, boundary))
        .unwrap_or(njutest::report::Decided::Survived)
}
...
fn disposed(id: &str, outcome: &str, reused: bool) -> MutantRecord {
    MutantRecord {
        ...
        reuse: njutest::report::Reuse(if reused {
            njutest::report::Established::ReadBackFrom("an-earlier-run".to_owned())
        } else {
            njutest::report::Established::Here
        }),
        blind_in: Vec::new(),
        routing: None,
    }
...
fn a_target_silent_in_one_part_and_noticing_in_another_is_accused_by_neither_nor_by_the_whole() {
    ...
    assert!(
        njutest::report::hollow::found(&union).is_empty(),
...
fn a_target_that_noticed_nothing_in_the_whole_catalog_is_accused_by_the_merge() {
    ...
    let mut union = one_rows;
    union.extend(two_rows);
    let accused = njutest::report::hollow::found(&union);
        ...
        move |source: &mut BuildReport| {
            source.concurrency = vec![ConcurrencyRecord {
                ...
                explored: njutest::report::concurrency::Exploration::Unexplored {
                    why: njutest::report::concurrency::Unexplored::NotAsked,
                },
            }];
        }
    };
