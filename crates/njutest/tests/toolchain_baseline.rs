...
use njutest::report::TargetStatus;
use njutest::trace::{Clock, MemorySink, Recorder, Sink, StartRecord};
...
fn a_workspace_that_does_not_compile_is_a_finding_and_not_an_error() {
    let broken = njutest::error::RunnerError::from(rust_mutants::EngineError::from(
    ...
    let elsewhere = njutest::error::RunnerError::from(rust_mutants::EngineError::from(
...
fn every_target_the_baseline_reads_is_one_a_person_watching_is_told_about() {
    ...
    let baseline = observe(
        &session,
        Reporting {
            notes: &mut njutest::ui::Notes::Plain(&mut said),
            watch: Watch::new(&cancel, &trace),
        },
    )
    .expect("the fixture baseline is read");
