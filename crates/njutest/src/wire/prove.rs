...
///
/// Cutting an answer short keeps everything up to and including the blank line that ends its head, so an answer that is all head comes back byte for byte the same; restating a status line as what it already says writes the line that is already there.
/// Nothing that reads bytes can tell either pair apart,
/// which is a stronger answer than any run: not "no test noticed" but "no observer could".
/// Every other question changes what the caller is handed, or when, so nothing is claimed about it here — a proof layer that guessed would report a gap as assured, which is the one answer this must never give.
#[must_use]
...
///
/// Asked of the function that does the restating, so the proof is a check on the injection rather than a second opinion about it.
/// A 500 whose reason phrase differs from the one a run writes is a different answer, and a proof that held only most of the time would not be a proof.
