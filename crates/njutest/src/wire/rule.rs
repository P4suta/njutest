...
///
/// A closed set rather than a name, because the catalogue that proposes a question and the interposer that puts it are two lists that would otherwise drift: a name added to one and not the other used to leave a question that went past untouched while the report said it had been put.
/// The fallback that caught it is gone, because the state it caught cannot be written.
#[derive(
    Debug,
    Clone,
    Copy,
