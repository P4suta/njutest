...
pub enum Sink {
    ...
    Memory(MemorySink),
    /// A durable directory that must keep every event, observed by an in-memory ring whose success can never hide a failed durable write.
...
impl Sink {
    ...
    /// Makes `authority` the only durable authority while retaining a best-effort ring for an in-process diagnostic bundle.
    #[must_use]
    ...
    /// How many events this sink lost, when it is the authority on that: a full ring absorbs a loss without an error, and only the sink knows.
    ...
    /// Called once by the recorder at the end of the recording; closing twice is not an error.
    ///
    /// # Errors
...
/// Keeps the most recent events in memory: the sink of a test, of an in-process reader, and of an untraced run's ring.
...
impl MemorySink {
    ...
    /// Offers an event to the non-authoritative diagnostic ring.
    /// A loss is retained as [`ObserverState::Corrupt`] or in its checked drop count,
...
impl DirSink {
    ...
    /// An explicit trace caller treats this as a setup failure; an unrequested trace uses only its in-memory ring.
