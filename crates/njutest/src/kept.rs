...
/// a constant spelling a structure is a layout every holder decides for itself, including the tests, and a layout the tests have decided is one no configuration can move.
#[must_use]
...
///
/// # Errors
/// The ledger could not be written, which never fails a run: what it is for is telling a person what is on their disk.
pub fn record(
    root: &Path,
    run_id: &str,
    at: Timestamp,
...
/// Removes from the ledger every directory that is no longer there, and returns what is left.
/// A directory that went away without this program's help is not something to keep telling a person about.
#[must_use]
