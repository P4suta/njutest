...
/// The domain separator hashed first for every target identity.
/// It carries the recipe version.
...
/// Every constructed or deserialized value is exactly 64 lowercase hexadecimal characters.
...
/// How long listing a binary's tests may take.
/// Listing is a process start and a print; a binary that has not answered by now is not going to.
...
/// # Errors
/// Returns [`TargetIdError::FieldTooLong`] if this platform can represent a string too large for the stable 64-bit length frame.
pub fn target_id(
    package: &str,
    unit: UnitKind,
    unit_name: &str,
...
///
/// # Errors
/// Returns the first invalid UTF-8 byte or line that is not exactly a known libtest entry.
/// An entirely empty stream is a valid listing with no tests;
