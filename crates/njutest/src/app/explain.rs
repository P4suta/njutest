...
///
/// Reading "somebody accepted it" off the absence of a finding says a person signed this off when all that is known is that nothing complained, which is also what a shard, a suppressed kind and a merged report look like.
/// The acceptances are in the configuration, so the question is asked of them, and where there is no answer the reader is told how to write one.
