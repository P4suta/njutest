...
/// One stored run whose report directory remains bound to the held store capability that selected it.
#[derive(Debug)]
...
impl ResolvedRun {
    ...
    /// The configuration read through the same held workspace capability that selected this run.
    #[must_use]
...
///
/// # Errors
/// [`RunError::NotFound`] when the document is not there and [`RunError::Unreadable`] when it is not one this version understands.
