...
///
/// The directory carries the document version, so a later contract gets its own and no reader has to guess which one an entry was written under.
...
impl Store {
    ...
    ///
    /// # Errors
    /// [`CacheError::Corrupt`] when an entry is there and is not that answer: a stored document that does not parse, does not carry the identity it is filed under, or does not satisfy the audit every durable report must.
    /// Nothing is silently ignored, because an entry that is quietly wrong is exactly what a wrong answer looks like.
    ...
    ///
    /// # Errors
    /// [`CacheError::Refused`] for a report that must not be stored: one with no identity, and one that was itself read back, because a chain of copies is not a chain of evidence.
    ...
    ///
    /// # Errors
    /// The store's own directory could not be listed, or an entry selected for removal could not be removed.
    ...
    ///
    /// # Errors
    /// [`CacheError::Corrupt`] for an entry that is not the answer it claims to be, [`CacheError::Unusable`] for a store that cannot be listed, and [`CacheError::Carrying`] for a destination that stops taking bytes.
    ...
    ///
    /// # Errors
    /// [`CacheError::Arriving`] for a line that is not a report, naming the line, [`CacheError::Refused`] for an answer this machine may not keep,
