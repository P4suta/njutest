...
impl Lease {
    /// Releases the claim.
    /// Idempotent, and never removes anything.
    ///
    /// # Errors
...
/// Takes the claim on `path` without waiting.
/// `Ok(None)` means somebody else has it.
///
/// # Errors
...
/// Takes the claim on `path`, waiting for whoever has it.
/// `waiting` is called once, the first time the claim is contended, so a command line can say what it is waiting for.
///
/// # Errors
/// See [`LeaseError`].
pub fn claim(
