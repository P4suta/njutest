...
///
/// An identity is a function of the whole file, so the edit somebody makes next — the test that closes this very survivor — re-mints it.
/// A locator is where the mutation is and what was done to it, which survives that edit.
#[must_use]
...
///
/// One function rather than one per command, because a name a run printed has to work everywhere a name is taken: `explain`, `accept` and `replay` each resolving it their own way is how a tool comes to print a command it then refuses.
#[must_use]
