...
/// The prefixes a run composes for itself.
/// A provider that sets one of these decides what every test process measures.
...
pub enum ResourceError {
    ...
    /// Starting one resource failed, then releasing resources already started for the same run also produced one or more failures.
...
///
/// Stopping asks each provider politely and then kills it, both bounded by that resource's own timeout.
/// A run holding several wedged providers would spend that twice per provider before it could exit, so the whole release is bounded too: what it does not reach politely is killed at once and reported, rather than making the run wait for something it is leaving anyway.
...
impl Drop for Manager {
    ...
    ///
    /// A provider is a child process holding something the operating system owns — a database, a container, a port.
    /// Every early exit between starting one and releasing it would otherwise leave it running with nothing naming it, and a build that fails leaves one behind per attempt.
    fn drop(&mut self) {
...
impl Manager {
    ...
    ///
    /// # Errors
    /// Every failure of the provider, and an environment a run composes itself.
    pub fn start(
        &mut self,
        capability: &str,
        resource: &Resource,
