...
///
/// An acceptance with no reason is a mutation nobody looked at, recorded as one somebody did.
/// Held as a type rather than checked at the point of use so that every path to an acceptance goes through the same refusal.
...
///
/// No `Accept`, and that is the whole point of the second type.
/// An acceptance says somebody looked at what a run found and decided it may stand; where nothing was found there is nothing to have looked at, and accepting one would record a decision about a measurement that never happened (ADR 0023).
/// The arm does not exist, so the loop cannot offer it and a caller cannot answer it.
...
///
/// Two methods rather than one, because there are two questions and their answers are different types.
/// A reviewer cannot accept what the run established nothing about, because the method that would carry the answer returns a type with no such arm (ADR 0023).
///
/// A trait rather than two closures so that one reviewer holds one terminal and one keyboard: two closures each wanting the same stream is a borrow the compiler refuses, and it is right to.
pub trait Answers {
...
///
/// The answers are an argument, so every rule of the loop is asserted in an ordinary test with no terminal and no keyboard — the shape `njutest watch` already uses for `look` and `round` (ADR 0020).
...
///
/// The same drawing the whole run gets, narrowed to one mark.
/// A review that showed a different picture from the report would be a second account of the same fact, which is what `Told` exists to prevent.
