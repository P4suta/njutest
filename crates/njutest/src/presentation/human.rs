...
///
/// A limitation is not a gap and does not compete with one for a reader's attention, but a run that passed over it silently would be claiming something it did not establish.
...
///
/// The source is drawn once and the marks go under the lines they are on, so a reader sees the shape of the blindness rather than reconstructing it from a list of entries that each quote one line.
...
///
/// The counts go beside the name while there is room for them and under it when there is not, rather than off the edge, where a narrow terminal breaks them wherever they happen to reach.
...
///
/// A command goes on its own line rather than being folded when it will not fit beside what it answers: a wrapped command is one nobody can select.
...
///
/// One number over three kinds would say they are one fact.
/// They are one drawing, because they are in one item and a reader reads it once, and they are not one fact, because each kind asks for different work.
...
///
/// The last one closes the drawing, which is how a reader's eye finds where one diagnostic ends and the next begins without counting blank lines.
...
///
/// A headline that ran past the edge was the one line nothing folded, so a narrow terminal wrapped it wherever it happened to reach and the two halves of a verdict ended up in different places.
...
///
/// A place the run could not measure is not a gap in somebody's tests, and a headline that counted it as one would report a busy machine as a hole in their suite.
/// `Standing` keeps the two apart, so this counts rather than remembers (ADR 0023).
