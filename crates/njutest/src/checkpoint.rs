...
impl State {
    ...
    ///
    /// [`SavedDisposition`] has no inconclusive arm, so a caller cannot put a timeout or finite step boundary into continuation evidence.
...
pub struct SavedTarget {
    ...
    /// The files it reached, and not the regions inside them.
    /// A restored target keeps reaching its whole file.
...
pub enum CheckpointViolation {
    ...
    /// A stored file cannot describe zero attempts: no attempt would have written it.
...
///
/// # Errors
/// [`CheckpointError::Corrupt`] when a file is there and is not the state it claims to be: one that does not parse, or one filed under a different identity.
/// Continuing from a checkpoint that is about different inputs would make a run claim what it never established.
...
/// Removes the checkpoint for `identity`.
/// A run that finished has nothing to continue from.
///
/// # Errors
/// [`CheckpointError::Unusable`] when an existing checkpoint or its now-empty identity directory cannot be removed.
/// Absence is success: interrupted runs need not have established a reusable fact.
