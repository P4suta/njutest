...
pub struct Checking<'a> {
    ...
    pub steps: u64,
    /// How long the build may take, which is not how long a measurement may take.
    /// `None` is no bound.
...
///
/// # Errors
/// Only what stopped the check from happening at all: a snapshot that could not be taken, a toolchain that could not be reached.
/// A candidate that does not work is a verdict, not an error.
pub fn check(
    checking: &Checking<'_>,
    proposal: &Proposal,
    mutant: &str,
