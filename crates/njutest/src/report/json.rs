...
///
/// # Errors
/// [`ReportError::Unsound`] when the report fails its own audit, and [`ReportError::Unserializable`] when the model itself cannot be written.
...
///
/// # Errors
/// [`ReportError::Unserializable`], which is an invariant failure rather than anything about the run.
...
///
/// # Errors
/// [`ReportError::Unreadable`] for a document with an unknown field, a missing field, or a value of the wrong shape; [`ReportError::Unsound`] for a shape-correct document whose facts contradict one another.
