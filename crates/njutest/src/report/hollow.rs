...
struct Answering {
    /// One witness for every mutation it gave an answer to.
    /// The count is the allocation's exact length, not an independently maintained integer.
    answered: Vec<()>,
...
///
/// The evidence is `routing.answered` and nothing else.
...
/// A part of one has seen a slice of the mutations, and a target silent in this part may have noticed something in another; the caller is what knows which it holds.
#[must_use]
