...
impl Sentence {
    ...
    ///
    /// A sentence nothing was asked about is neither held up nor a gap.
    /// The tests were never given the chance, so counting it among the ones nothing would notice changing would tell a reviewer the suite is blind where the run established nothing at all.
    #[must_use]
...
///
/// The sentences are made of what went past and nothing else.
/// A specification derived this way is always true of the system as it ran, which is what makes the annotation worth reading: the question is never whether the behaviour is real, only whether anybody would notice it changing.
/// # Errors
/// Returns [`super::CountError`] if one exchange's exact question census cannot be represented by the v1 sentence counters.
...
/// # Errors
/// Returns the checked projection error instead of shortening a sentence's question census.
...
/// nobody could have noticed it, so counting it against the tests would ask them for something no test can give.
