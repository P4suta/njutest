...
///
/// # Errors
/// [`RepairErrorKind::Protocol`] for an answer this version does not understand, [`RepairErrorKind::PathRefused`] for a path outside what the configuration allows, and [`RepairErrorKind::PreimageMoved`] when the file a candidate patches is not the file the provider saw.
...
/// The allowed paths as patterns.
/// A pattern that is not one allows nothing rather than everything.
