...
/// Directories every project writes rather than reads.
/// Nothing under them is part of what the tests are about.
...
///
/// The report directory used to be a word in the list above, so a project that moved it digested its own reports into the run identity, watched itself write them, and asked git about them.
/// Where a project writes is configuration, so the list is a value rather than a constant.
#[derive(Debug, Clone)]
...
impl Excluded {
    ...
    ///
    /// # Errors
    /// Refuses a report directory whose exact platform spelling cannot enter the UTF-8 evidence and git-exclusion protocols.
...
impl Scan {
    /// The digest of everything under `prefix`, as a slash-separated workspace-relative directory.
    /// A directory with nothing under it folds to a digest of its own, which is not the digest of a directory with something under it.
    #[must_use]
...
pub enum ScanError {
    ...
    /// A platform path cannot be represented byte-for-byte by the UTF-8 evidence protocol.
    #[error(
...
pub enum Entry {
    ...
    /// Its target is what enters a digest, never what it points at.
    Link(String),
