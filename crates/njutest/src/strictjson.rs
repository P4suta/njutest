...
/// Decodes one strict JSON document into its typed representation.
///
/// # Errors
/// The bytes are not unique-key JSON or do not satisfy `T`.
#[expect(
...
/// Decodes one strict JSON byte stream into its typed representation.
///
/// # Errors
/// The bytes are not UTF-8 unique-key JSON or do not satisfy `T`.
#[expect(
    clippy::redundant_pub_crate,
    reason = "the compiler-surface harness re-exports this crate-visible boundary"
)]
...
/// Parses one complete JSON value and rejects duplicate object keys instead of accepting `serde_json`'s last-key-wins projection.
///
/// # Errors
/// The text is not one complete unique-key JSON value.
#[expect(
    clippy::redundant_pub_crate,
    reason = "the compiler-surface harness re-exports this crate-visible boundary"
)]
...
/// Parses one complete UTF-8 JSON byte stream and rejects duplicate object keys at every depth.
///
/// # Errors
/// The bytes are not one complete UTF-8 unique-key JSON value.
#[expect(
    clippy::redundant_pub_crate,
    reason = "the compiler-surface harness re-exports this crate-visible boundary"
)]
pub(crate) fn from_slice(bytes: &[u8]) -> Result<serde_json::Value, serde_json::Error> {
...
///
/// # Errors
/// The explicitly present value cannot deserialize as `Option<T>`.
#[expect(
    clippy::redundant_pub_crate,
    reason = "the compiler-surface harness re-exports this crate-visible boundary"
)]
...
mod tests {
    ...
    fn duplicate_keys_are_rejected_recursively() {
        for text in [
            ...
            let error = super::from_str(text).expect_err("duplicates never become a value");
            assert!(
