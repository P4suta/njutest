...
fn body<T: serde::de::DeserializeOwned>(
    ...
    let placed = |cause| crate::route::ReadError { line, cause };
    let seq = crate::route::required(event, "seq", Value::as_u64).map_err(placed)?;
    let record = crate::route::required(event, kind, Some).map_err(placed)?;
    let read = serde_json::from_value(record.clone())
        .map_err(|source| placed(crate::route::ReadCauseError::Json { source }))?;
    Ok((seq, read))
}
...
///
/// # Errors
/// A control, confirmation or inherited kill that is not the shape the reader expects, or has no whole sequence number.
pub fn read(
    recorded: &crate::route::Checked<crate::schemas::RunnerLines>,
) -> Result<Confirmations, crate::route::ReadError> {
    let mut read = Confirmations::default();
    for (at, event) in recorded.events().iter().enumerate() {
        let line = at.saturating_add(1);
    ...
    pub fn decided(&self, seq: u64, confirm: &Confirm) -> Decided {
        let Some((_, control)) = self.controls.iter().rev().find(|(at, control)| {
            ...
            *at < seq
            ...
            return Decided::Broke(
                ...
                format!(
                    "its confirmation rests on a control asked for {} that the recording does not hold before it",
        ...
        match (&control.answer, confirm.reproduced) {
            (Answer::Failed { .. }, Some(outcome)) => Decided::Broke(
                ConfirmRule::Rerun,
                format!(
                    "its control failed on the original code, and the recording says it was run a second time anyway and came to {outcome:?}"
                ),
            ),
