...
pub fn invariants(root: &Path) -> Result<String, GateFailure> {
    let read = |relative: &str| {
        std::fs::read_to_string(root.join(relative))
            .map_err(|error| GateFailure(format!("invariants: {relative}: {error}")))
    };
    let coded = |error: crate::invariants::InvariantError| {
        GateFailure(format!("invariants: {}", error.coded()))
    };
    ...
    for path in all_sources(root)? {
        let text = std::fs::read_to_string(&path)
            .map_err(|error| GateFailure(format!("invariants: {}: {error}", path.display())))?;
