...
/// `String + &str` is addition; `String - &str` is not subtraction. `add-to-sub` is refused with E0369.
pub fn greet(name: String) -> String {
    name + "!"
}

/// A range swap changes the type: `Range<usize>` is not `RangeInclusive`. `range-to-inclusive` is refused with E0308.
pub fn window(n: usize) -> std::ops::Range<usize> {
    0..n
}

/// `value * 0` swapped to `value / 0` is refused by `unconditional_panic`, which is deny by default. The refusal only happens once code is generated: `cargo check` accepts it, and `cargo test --no-run` does not. Validation therefore compiles the way the run runs.
