...
| `exec` | one command: argv verbatim, dir, environment variable **names**, timeout, how it stopped — an exit code of its own, a bound that expired, a guard's count passed, or never started — duration, output digest and preserved-output path |
