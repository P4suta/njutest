...
| `crash-exec` | `crash`, `target`, `test`, `stage`, `exit_code`, `outcome`, `noticed`, `left`, `failed` | one run of a test a crash was put to: `crash`, stopped at the call where `noticed` says the runtime published the notice of that stop, with the files and directories it `left`; `next`, over what a stop left; `fresh`, in a scratch of its own, with the tests either `failed`; every crash's decision is re-derived from these and its `crash-step` records in order |
