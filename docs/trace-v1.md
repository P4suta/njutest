...
| `sentinel` | `layer`, `mutant`, `expected`, `routed`, `sighted` | one mutant planted for a routing layer before the baseline: `layer` is `reach`, `branch-never-taken`, or `never-infected`; `expected` is `unreached`, `kept`, or the proof that must discharge it; `routed` is what the engine decided, as a reader is told; a run ends in `ERROR` (`NJ5009`) after the first one not `sighted` |
