...
and `not-measured` with one closed `why`: `no-control` (nothing confirmed a kill on it), `unrecorded`, `unreadable`, `control-failed`, `other-tests`, `no-baseline`, or `baseline-retried`, which a target whose baseline passed only when run again in the directory its failed first attempt left gets, because that run did not happen under the conditions a control's does.
