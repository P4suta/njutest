...
`reached-v1.json` (`schema/rust-mutants-reached-v1.json`), `skeletons-v1.json` (`schema/rust-mutants-skeletons-v1.json`) and `catalog-v1.json` beside its report.
...
which says so, the body digests, sealing and unit skeletons an answer would be carried across an edit by ([carrying an answer](carry.md)), and the catalog with the branch bodies the compiler vouched for.
