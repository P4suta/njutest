...
| `exec` | `argv`, `dir`, `env_names`, `timeout_ms`, `quiet_ms`, `stopped`, `duration_ms`, `output_bytes`, `output_sha256`, `output_truncated`, `output_path`, `error` | every process: argv verbatim, dir, environment variable names, timeout (the ceiling, for a process watched for progress), the quiet window it was watched for or null, and one closed stop reason — not started, with why (`missing`, `denied`, `busy`, `exhausted`, `unsupervised`, `malformed`, `unprepared`, `not-asked`, or `other` with the operating system's words), its own code/signal/unknown exit, timed out, stalled with no step raised for a whole quiet window, cancelled, wait failed, a verified step-limit notice, a failed step protocol, or answered: a mutant's test process ended at the first test libtest said failed, since that one failure is the whole answer — plus duration, output digest and preserved output. A process cannot be both timed out and exited |
...
| `verify` | `target`, `outcome`, `tests_run`, `duration_ms`, `args`, `remembered`, `retried`, `declined` | per target: what the suite established with nothing active, how many tests ran, how long the baseline took, the harness arguments it ran with, whether that exact passing measurement was remembered, whether it retried, and each test that declined to measure on this machine with its words, which are the only declines a mutant execution of the target is excused ([ADR 0043](../adr/0043-a-test-may-decline-to-measure.md)) |
| `perturbed-control` | `target`, `perturbation`, `outcome`, `failed_tests`, `duration_ms`, `reach` | one control started under a perturbation: each variable set over its environment with its value (`null` where the value is not text), what the shell it was started through ran first, the harness arguments its schedule added, the guard it paused its threads at (`delay`) or the guard whose delayed failure it is the undelayed half of a confirming round for (`confirms`), what it came to and which tests failed, and what became of its reach, closed by `state`: `not-asked` (it observed nothing, ran one test, or its guards cannot be asked), `not-read` (it did not pass), `unrecorded`, `unreadable`, or `recorded` with its `touch` record; kept apart from `touch` and `mutant-exec` so that nothing comparing a control with its baseline under equal conditions reads it |
| `touch` | `target`, `measured`, `mutant`, `passed`, `summary`, `tests`, `sites`, `loose`, `infected`, `entered`, `reached_sites`, `entered_bodies`, `infected_sites`, `entered_items` | per target and per whole run of it — `baseline`, or a `control` run to confirm a kill, each with nothing active and `mutant` null, or a `repair` that ran the full `mutant` against a target whose reach moved ([ADR 0036](../adr/0036-what-rested-on-a-moved-reach-is-run-again.md)): the tests that run passed, what its own summary said in the protocol it answered in (`libtest` with its count, `custom`, `unanswered`, or `remembered` for a baseline from an earlier session), how many of its tests reached a mutation, how many distinct mutations anything of it reached, how many were reached where nothing named a test, how many it saw a guard's two branches differ over, how many distinct items anything of it entered the body of, and the four unions themselves (sites, bodies, infected, entered items), which is what `xtask proofaudit` compares between the two runs ([ADR 0025](../adr/0025-a-reach-that-moves-is-not-a-measurement.md)) |
...
| `select` | `mutant`, `reason` | why one mutant was never executed, or measured nothing where it was (`declined`) |
...
| `mutant-exec` | `id`, `index`, `target`, `outcome`, `step_notice`, `exit_code`, `duration_ms`, `tests_run`, `signal`, `failed_tests`, `timeout_ms`, `timeout_source`, `alone`, `lingered`, `entered_records`, `declined` | the mutation, target, outcome, any verified step-limit notice, status, duration, tests run, signal, failed tests, the budget it was given and where that came from, whether it had the machine to itself, whether its harness had answered before the clock ended the process, how many records naming entered items it wrote when asked to (null when it was not), which is what recording that union cost (ADR 0041), and each test that declined to measure with its words, where its notice was believed (ADR 0043) |
