...
| `perturbed-control` | `target`, `perturbation`, `outcome`, `failed_tests`, `duration_ms`, `reach` | one control started under a perturbation: each variable set over its environment with its value (`null` where the value is not text), what the shell it was started through ran first, the harness arguments its schedule added, the guard it paused each thread at once (`delay`, `null` where none), what it came to and which tests failed, and what became of its reach, closed by `state`: `not-asked` (it observed nothing, ran one test, or its guards cannot be asked), `not-read` (it did not pass), `unrecorded`, `unreadable`, or `recorded` with its `touch` record; kept apart from `touch` and `mutant-exec` so that nothing comparing a control with its baseline under equal conditions reads it |
