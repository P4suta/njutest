...
- A test process is given a home inside its execution's scratch, `HOME` and the XDG directories under it, so a write a mutation redirects there is emptied with the scratch ([ADR 0044](adr/0044-a-test-writes-only-where-its-execution-may.md)); `CARGO_HOME` and `RUSTUP_HOME` stay where they are, and `CARGO_HOME` is writable.
  A target whose tests pass only with the home the run was given is `unconfined-target`: every execution of it runs with that home, where a mutation of it can write, and every run names it.
