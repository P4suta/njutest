...
- A test process is given a home of its execution's own, beside its temporary directory, with `HOME` and the XDG directories under it, so a write a mutation redirects there is emptied with the scratch ([ADR 0044](adr/0044-a-test-writes-only-where-its-execution-may.md)); `CARGO_HOME` and `RUSTUP_HOME` stay where they are, and `CARGO_HOME` is writable.
  A target whose tests fail in that home and pass with the one the run was given is `unconfined-target`: every execution of it runs with the given home, where a mutation of it can write, every run names it, and what it reached in a home of its own is not read, so every mutant routes to it.
  A variable the engine does not confine keeps the value the run was given, and one that names a place under the given home, as `MISE_DATA_DIR` may, is a way a test can still write there.
  Where the run was given no home at all, `CARGO_HOME` and `RUSTUP_HOME` are pinned only if they are set, and a test that runs cargo otherwise finds an empty cargo home of its own.
  On Windows a program that asks the shell for a known folder rather than reading `USERPROFILE`, `APPDATA` or `LOCALAPPDATA` is told the real one.
