...
| `claims` | a claim of `.rust-mutants.toml` that names nothing, or not as many mutations as it says, which a run would only find unmatched after it had measured everything else. It asks `rust-mutants list --claims`, the engine's own locator, rather than a second matcher, so the gate and a run cannot read a claim differently, and it first plants two claims in `fixtures/fixture-simple` and requires the one naming nothing to be refused and the one naming its mutation to pass before its silence about the repository is believed |
