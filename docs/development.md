...
| Gate | Refuses |
| --- | --- |
| `lints` | `allow-attribute`: an `#[allow]` anywhere in the repository, tests included. `boxed-trait-object`: a `Box<dyn Trait>`. `comment`: a comment that is not documentation, the licence header, or a `rust-mutants:` annotation the engine reads. `unbounded-removal`: a recursive removal inside a loop, which `rust_mutants::reclaim` does with a budget and an account of what is still there. `perishable-handle`: a command or a record built with a mutant identity in it, which the edit that closes a survivor re-mints |
