...
| `claims` | a claim of `.rust-mutants.toml` that names nothing, not as many mutations as it says, or a line its mutation is no longer on, which a run would only find after it had measured everything else; a line is what a reader goes to, so a claim holding a stale one says something that is not so even while it still names one mutation. It asks `rust-mutants list --claims`, the engine's own locator, rather than a second matcher, so the gate and a run cannot read a claim differently, and it first plants three claims in `fixtures/fixture-simple` and requires the one naming nothing and the one holding a stale line to be refused, and the one naming its mutation to pass, before its silence about the repository is believed |
...
Its other named layers are `killers`, `acceptances`, `reuse`, `hollow`, `model`, `repair`, and `dimensions`; each name is printed on its remarks and coverage line.
...
Runs that wait go in in the order they began to wait: each takes a ticket, and a run that died while it waited gives its place up (on Windows the lock alone decides).
...
The work a dead holder started answers to nobody, so the next holder asks its group to stop, kills it after ten seconds, and goes in only once it has ended, saying each step; a loop left running by a test killed midway once held the lane for every session on the machine while runs waited for it to end on its own.
