...
Each remark names the layer it is from: `accounting`, `killers`, `findings`, `acceptances`, `reuse`, `proofs`, `hollow`, `wire`, `model`, `drift`, `executions`, or `confirmations`, and a test holds that list to the layers the audit has.
...
The `confirmations` layer holds how each kill and wait was confirmed: the runner records a `control` the one time the original code answers a test and a `confirm` for each kill and wait, naming whose control answered and what the second run came to.
Each killed, waited, and unconfirmed row is held to the last confirmation of it against its target, which must rest on a control recorded before it for the same test and asker; a kill stands only on a passing control and a second run that came back, and a question asked of the original code twice is a violation, since a control is memoized per test.
Each violation names the `ConfirmRule` it breaks, and the gate finds a plant for every rule by name; a kill inherited from an interrupted run's checkpoint, which the recording marks `resumed`, is `unaudited` here because it was confirmed in that run's recording.
