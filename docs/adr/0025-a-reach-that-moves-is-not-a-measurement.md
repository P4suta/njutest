...
4. **The two runs are measured under equal conditions first.** A baseline ran with the run's shared scratch as its temporary directory, and a control with a fresh directory of its own.
   ...
   A target's baseline keeps that one directory across the processes it may take — the retry of a target that did not pass, and the rerun of one that could not record — because a retry that recovers from what its first attempt left is the behaviour `baseline-passed-on-retry` already names.
   ...
   Two such sets differing would let the parser raise a finding about a suite whose test set is deterministic, so under libtest each run's named tests are held to its own summary's count first, and a run that falls short on either side is `not-measured` with `unparsed`; a custom harness prints no summary and names no tests, so the test-set premise is empty for it and its unions are compared as they are; the baseline says `baseline-passed-unparsed`, and the audit reads both counts from the `touch` records.
...
6. **Absence is a case.** A target whose baseline was measured and that no comparable control recorded is `not-measured`, and the `drift-not-measured` limitation counts and names them.
   It is not `held`: a comparison nothing made is not one that agreed.
   Every target the baseline measured and no control confirming a kill compared is run whole once more after the mutation phase, alone, under the conditions a control runs under, so `no-control` is left for a run cancelled before it got there.
   It was first left `not-measured` instead, and `select` reads a target's standing to decide whether skipping it is sound, so a target that noticed nothing — the one a selection most wants to skip — would never have been skippable.
...
7. **Where it is recorded.** Each catalog part records one drift record per measured target — `held`, `moved` with what moved, or `not-measured` with why.
   ...
   A target whose every kill was inherited is run alone for the comparison, like one that confirmed no kill.
