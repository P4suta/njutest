...
A test that cannot measure here appends one line, `<its libtest name>\t<why>`, and returns.
...
**A baseline decline is kept, not believed.** A test that declines in the baseline still reached what it reached, and reach is only a promise that the test could notice.
The run records the decline, and routes nothing to that test on its strength alone: a mutation it alone reaches is `not_run`/`declined` without an execution.
...
- The trace records each decline, and engine-audit re-derives each `declined` fate from the recorded lines and the tests the route asked.
