...
4. **Under a mutant, it is attributed by when and by whom.** Mutant executions run side by side, so an orphan left during the mutation phase cannot be told apart by directory; the directory is cleared when the baseline ends, and an execution records when it started and ended.
   ...
   On Windows the runtime cannot name a parent, since the standard library offers one only on Unix, so each execution's job object is associated with a completion port that names every process the job takes in, grandchildren included.
   An orphan a container named is the execution's whose container named it, and the session drains every running execution's port before it decides, since a process is named when it is created and so before it can say it lost the environment.
   Windows does not promise to post every such message, so the membership is used only as evidence for an attribution: a process no container named falls back to the parent, and on Windows to counting as this execution's.
