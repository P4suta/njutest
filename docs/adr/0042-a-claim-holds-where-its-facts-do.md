...
Configured skips take the same resolution with the skip anchors that follow.
The declared environment enters the outcome key and carry's premises through `outcomes::Declared`, from `Config::declared_env`.
