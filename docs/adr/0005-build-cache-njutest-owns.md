...
1. **njutest owns its build cache, in two layers.** A *base* layer belongs to
...
4. **The rule lives in one function and is pinned by a test** that names
   every command njutest issues and which side of the rule it falls on.
5. **Every run bounds the base layer.** `[cache] build_max_bytes` bounds it —
   ...
   a non-blocking lock; `njutest cache gc` runs the same collection on demand.
6. **A layer is a directory njutest made, and the marker proves it.** A
   directory that exists, holds files, and carries none of njutest's names is
   refused without writing anything into it. The marker is
   `njutest-build-cache-v1`.
...
- A build of identical source at a different absolute path misses for the
  ...
  njutest does not remap paths — that would change the binaries under
...
- What a run asked the cache for is reported as a progress note, so a reader
  who sees njutest go faster can see how much of it was the cache — the same
