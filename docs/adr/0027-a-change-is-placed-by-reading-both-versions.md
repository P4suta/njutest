...
- A target that reads an item file as data — `include_str!` of a module, a test reading `src/` — reads the instrumented bytes in every run the engine makes, so its baseline may fail where the pristine tree passes, and its measurement says nothing about the file's text; such a target is a tree-reader, which `njutest measure` finds by running it once more without those files and marks `reads-tree`, so a selection never skips it.
...
- The measurement records what this module cannot see for itself: every file of the tree by digest and mode, every dep-info input outside it, the values behind `env-dep` lines, the files compiled into a procedural macro or a build script (an edit to one is `Everything::CompileTime`), the build selection and harness arguments, the targets that read the tree as data, and the targets that start a process without the run's environment.
  What a build script reads at run time and names only through `rerun-if-changed` is not in any dep-info, and is covered only because the script itself is compile-time and a file outside every unit runs everything.
