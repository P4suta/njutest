...
- A target that reads an item file as data — `include_str!` of a module, a test reading `src/` — reads the instrumented bytes in every run the engine makes, so its baseline may fail where the pristine tree passes, and its measurement says nothing about the file's text; such a target is a tree-reader, which the measurement has to find by running it without those files.
