...
//! `.mjutest.toml` is strict, and strictness is exactly what a hostile document tests: a configuration this parser *accepts* decides what a run does, so it must never accept something it cannot then describe, and it must never panic deciding.
